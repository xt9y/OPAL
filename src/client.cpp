...
void run_sdl_control(SessionSupervisor&session,VideoPresenter&presenter){
    ...
    while(run&&session.running()){
        bool did_work=false;DecodedVideoFrame frame{};if(session.take_latest_video(frame)){did_work=true;if(!present_frame(session,presenter,frame)){std::cerr<<"OPAL presenter failed error="<<SDL_GetError()<<"\n";break;}}
        SDL_Event event{};
...
int client_connect(const std::string&target_in,const std::string&password_arg,const StreamOptions&stream){
    ...
    DecodedVideoFrame first{};for(int i=0;i<100&&!first.frame&&session.running();++i){session.take_latest_video(first);if(!first.frame)std::this_thread::sleep_for(std::chrono::milliseconds(10));}
    ...
    run_sdl_control(session,presenter);presenter.close();session.stop();quit_sdl();return 0;
}
