...
std::string pairing_code(){static const char*a="ABCDEFGHJKLMNPQRSTUVWXYZ23456789";unsigned char r[16];if(RAND_bytes(r,sizeof(r))!=1)throw std::runtime_error("RAND_bytes failed");std::string s;s.reserve(19);for(int i=0;i<16;i++){if(i>0&&i%4==0)s+='-';s+=a[r[i]&31];}return s;}
std::string normalize_pairing_code(std::string code){
    if(code.size()!=19||code[4]!='-'||code[9]!='-'||code[14]!='-')return code;
    for(size_t i=0;i<code.size();++i){
        if(i==4||i==9||i==14)continue;
        unsigned char ch=static_cast<unsigned char>(code[i]);
        if(!std::isalnum(ch))return code;
        code[i]=static_cast<char>(std::toupper(ch));
    }
    return code;
}
