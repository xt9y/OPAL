...
struct VideoPresenter::Impl{
    ...
    bool render_texture(int source_width,int source_height){const SDL_FRect dst=fitted_rect(drawable_width,drawable_height,source_width,source_height);const auto present_begin=Clock::now();if(!SDL_RenderClear(renderer)||!SDL_RenderTexture(renderer,texture,nullptr,&dst)||!SDL_RenderPresent(renderer))return false;const auto present_end=Clock::now();present_latency.push(elapsed_ms(present_begin,present_end));++presented;debug_timing();return true;}
#if defined(__APPLE__)
    bool present_videotoolbox_frame(const AVFrame*source){
        if(!source||!source->data[3])return false;
        auto pixel=reinterpret_cast<CVPixelBufferRef>(source->data[3]);
        const OSType cv_format=CVPixelBufferGetPixelFormatType(pixel);
        if(cv_format!=kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange&&cv_format!=kCVPixelFormatType_420YpCbCr8BiPlanarFullRange)return false;
        if(!CVPixelBufferIsPlanar(pixel)||CVPixelBufferGetPlaneCount(pixel)<2)return false;
        const int width=static_cast<int>(CVPixelBufferGetWidthOfPlane(pixel,0));
        const int height=static_cast<int>(CVPixelBufferGetHeightOfPlane(pixel,0));
        if(width<=0||height<=0||!ensure_texture(width,height,AV_PIX_FMT_NV12)||!refresh_drawable_size())return false;
        ...
        const auto*y=static_cast<const std::uint8_t*>(CVPixelBufferGetBaseAddressOfPlane(pixel,0));
        const auto*uv=static_cast<const std::uint8_t*>(CVPixelBufferGetBaseAddressOfPlane(pixel,1));
        const int y_pitch=static_cast<int>(CVPixelBufferGetBytesPerRowOfPlane(pixel,0));
        const int uv_pitch=static_cast<int>(CVPixelBufferGetBytesPerRowOfPlane(pixel,1));
        const bool uploaded=y&&uv&&y_pitch>0&&uv_pitch>0&&SDL_UpdateNVTexture(texture,nullptr,y,y_pitch,uv,uv_pitch);
        CVPixelBufferUnlockBaseAddress(pixel,kCVPixelBufferLock_ReadOnly);
        const auto upload_end=Clock::now();
        if(!uploaded)return false;
        upload_latency.push(elapsed_ms(upload_begin,upload_end));
        return render_texture(width,height);
    }
#endif
    ...
    bool present_frame(const AVFrame*source){
#if defined(__APPLE__)
        if(source&&source->format==AV_PIX_FMT_VIDEOTOOLBOX)return present_videotoolbox_frame(source);
#endif
        const AVFrame*f=nullptr;if(!normalize_frame(source,f)||!f||!ensure_texture(f->width,f->height,f->format)||!refresh_drawable_size())return false;const auto upload_begin=Clock::now();bool uploaded=false;if(f->format==AV_PIX_FMT_YUV420P||f->format==AV_PIX_FMT_YUVJ420P)uploaded=SDL_UpdateYUVTexture(texture,nullptr,f->data[0],f->linesize[0],f->data[1],f->linesize[1],f->data[2],f->linesize[2]);else if(f->format==AV_PIX_FMT_NV12)uploaded=SDL_UpdateNVTexture(texture,nullptr,f->data[0],f->linesize[0],f->data[1],f->linesize[1]);const auto upload_end=Clock::now();if(!uploaded)return false;upload_latency.push(elapsed_ms(upload_begin,upload_end));return render_texture(f->width,f->height);}
