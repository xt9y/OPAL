...
int interactive_run(const StreamOptions &stream){auto p=Paths::load();Ini cfg;if(!cfg.load(p.config)||cfg.get("opal","role").empty())return first_setup(stream);auto role=cfg.get("opal","role");if(role=="host")return ensure_host_service();if(role=="client"){int rc=connect_default(cfg,stream);if(rc==-1)return first_setup(stream);return rc;}return first_setup(stream);}
}
