...
int interactive_restart(){auto p=Paths::load();Ini cfg;if(!cfg.load(p.config)||cfg.get("opal","role").empty())return first_setup();const auto role=cfg.get("opal","role");if(role=="client")return interactive_run();if(role=="host"){if(require_tailscale()!=0)return 1;return restart_services();}return first_setup();}
