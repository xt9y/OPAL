...
static Paths G;
static Ini host_cfg;
static std::atomic<bool> test_control_closed{false};
static int host_desktop_width=0,host_desktop_height=0;
...
static std::pair<int,int> desktop_geometry(){
    Display*d=XOpenDisplay(nullptr);
    if(!d)return {0,0};
    int screen=DefaultScreen(d);
    int width=DisplayWidth(d,screen),height=DisplayHeight(d,screen);
    XCloseDisplay(d);
    return width>0&&height>0?std::pair<int,int>{width,height}:std::pair<int,int>{0,0};
}

static void append_test_log(const char*env_name,const std::string&line){
    const char*path=std::getenv(env_name);
    if(!path||!*path)return;
    std::lock_guard<std::mutex>l(test_log_mu);
    std::ofstream out(path,std::ios::out|std::ios::app);
    out<<line<<'\n';
}

static int test_control_close_after_pings(){
    const char*value=std::getenv("OPAL_TEST_CONTROL_CLOSE_AFTER_PINGS");
    if(!value||!*value)return 0;
    try{int n=std::stoi(value);return n>0?n:0;}catch(...){return 0;}
}
...
static void control_client(TlsConn c){
    if(!c.ssl)return;
    // Keep geometry inside the opaque signed challenge. Old clients already
    // sign the full suffix, while new clients can parse the trailing size.
    std::string challenge=random_hex(32);
    if(host_desktop_width>0&&host_desktop_height>0)challenge+=" "+std::to_string(host_desktop_width)+" "+std::to_string(host_desktop_height);
    tls_write_line(c.ssl,"CHALLENGE "+challenge);
    std::string line;
    if(!tls_read_line(c.ssl,line)){close_tls(c);return;}
    std::istringstream ss(line);std::string mode,pub,proof,label;ss>>mode>>pub>>proof;std::getline(ss,label);label=trim(label);
    bool ok=false;
    if(mode=="AUTH"&&authorized(pub))ok=verify_hex(pub,challenge,proof);
    else if(mode=="PAIR"){
        auto password=host_cfg.get("host","password");ok=secure_equal(proof,hmac_sha256_hex(password,challenge+pub));
        if(ok)authorize(pub,label.empty()?"client":label);
    }
    if(!ok){tls_write_line(c.ssl,"DENY");close_tls(c);return;}
    append_test_log("OPAL_TEST_AUTH_LOG",mode);
    auto token=random_hex(24);
    {std::lock_guard<std::mutex>l(session_mu);sessions.insert(token);}
    if(!tls_write_line(c.ssl,"OK "+token)){
        std::lock_guard<std::mutex>l(session_mu);sessions.erase(token);close_tls(c);return;
    }
    HeldInputState held;
    int ping_count=0;
    while(tls_read_line(c.ssl,line)){
        if(line=="PING"){
            ++ping_count;
            int threshold=test_control_close_after_pings();
            bool expected=false;
            if(threshold>0&&ping_count>=threshold&&test_control_closed.compare_exchange_strong(expected,true)){
                if(c.fd>=0)shutdown(c.fd,SHUT_RDWR);
                break;
            }
            if(!tls_write_line(c.ssl,"PONG"))break;
            continue;
        }
...
static void video_client(TlsConn c){
    ...
    if(!ok){tls_write_line(c.ssl,"DENY");close_tls(c);return;}
    append_test_log("OPAL_TEST_VIDEO_TOKEN_LOG",token);
    int fps=host_cfg.get_int("video","fps",60),bitrate=host_cfg.get_int("video","bitrate_kbps",12000);bool audio=host_cfg.get_bool("audio","enabled",true);bool gsr=command_exists("gpu-screen-recorder");
    ...
    if(!tls_write_line(c.ssl,"READY")||!tls_write_all_timeout(c.ssl,buf,static_cast<size_t>(n),video_backpressure_timeout_ms)){stop_capture(capture);close_tls(c);return;}
    for(;;){n=read_capture(capture,buf,sizeof(buf),3000);if(n<=0)break;if(!tls_write_all_timeout(c.ssl,buf,static_cast<size_t>(n),video_backpressure_timeout_ms))break;}
