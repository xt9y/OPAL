...
bool input_send(const std::string&s){const auto mapped=remap_pointer_line(s);const auto display_key=current_input_display_key();std::lock_guard<std::mutex>l(input_mu);if(display_key!=input_display_key){if(input_sink.pid>0||input_sink.fd>=0)input_close_locked();input_display_key=display_key;if(debug_enabled())std::cerr<<"OPAL input display="<<display_key<<"\n";}if(input_write_locked(mapped))return true;return input_write_locked(mapped);}
