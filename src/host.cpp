...
std::mutex auth_mu,input_mu,test_log_mu;SinkProcess input_sink;std::string input_display_key;Paths G;Ini host_cfg;int host_desktop_width=0,host_desktop_height=0;std::atomic<bool>test_close_used{false};
...
std::string current_input_display_key(){const auto d=active_host_display();if(!d.valid)return"none";return d.backend+"|"+std::to_string(static_cast<int>(d.kind))+"|"+std::to_string(d.mode.width)+"x"+std::to_string(d.mode.height);}
...
bool input_send(const std::string&s){const auto mapped=remap_pointer_line(s);
#if defined(OPAL_PLATFORM_WINDOWS)
    if(windows_input_send(mapped))return true;
#endif
    const auto display_key=current_input_display_key();std::lock_guard<std::mutex>l(input_mu);if(display_key!=input_display_key){if(input_sink.pid>0||input_sink.fd>=0)input_close_locked();input_display_key=display_key;if(debug_enabled())std::cerr<<"OPAL input display="<<display_key<<"\n";}if(input_write_locked(mapped))return true;return input_write_locked(mapped);}
...
bool run_peer_session(TailnetHostResult direct,const std::string&host_public_key,HostClipboardBridge*clipboard){
    ...
    std::string last_meta=host_meta();peer.send_input(last_meta);auto next_meta=Clock::now()+std::chrono::milliseconds(250);if(debug_enabled())std::cerr<<"OPAL host peer path="<<peer.path_name()<<" session="<<direct.session_id.substr(0,8)<<"...\n";
    ...
    while(peer.running()){const auto now=Clock::now();if(test_close_this_session&&now>=test_close_at){peer.stop();break;}if(now>=next_meta){const auto meta=host_meta();if(meta!=last_meta){if(!peer.send_input(meta))break;last_meta=meta;}next_meta=now+std::chrono::milliseconds(250);}if(clipboard)clipboard->pump();std::this_thread::sleep_for(std::chrono::milliseconds(20));}if(clipboard)clipboard->detach(&peer);if(sender_start_thread.joinable())sender_start_thread.join();sender.stop();for(const auto&r:held.release_commands())input_send(r);peer.stop();return true;
}
