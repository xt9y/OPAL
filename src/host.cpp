...
void local_discovery_loop(const std::string&host_public_key,HostClipboardBridge*clipboard){
    std::string error;UdpSocket listener;
    for(;;){
        if(listener.fd<0){
            listener=open_local_discovery_listener(kLocalDiscoveryPort,"",error);
            if(listener.fd<0){if(debug_enabled())std::cerr<<"OPAL direct discovery bind failed reason="<<error<<"\n";std::this_thread::sleep_for(std::chrono::seconds(1));continue;}
            if(debug_enabled())std::cerr<<"OPAL direct discovery listening 0.0.0.0:"<<listener.local_port<<"\n";
        }
        ...
        if(!wait_local_client(listener,host_public_key,G.identity_key,local,500,error)){
            if(error!="local discovery timeout"){
                ...
                close_udp_socket(listener);std::this_thread::sleep_for(std::chrono::milliseconds(250));
            }
            continue;
        }
