...
bool authorized(const std::string&pub){std::lock_guard<std::mutex>l(auth_mu);std::ifstream f(G.authorized);std::string line;while(std::getline(f,line)){const auto p=line.find('|');if((p==std::string::npos?line:line.substr(0,p))==pub)return true;}return false;}
...
void authorize(const std::string&pub,const std::string&label){if(authorized(pub))return;std::lock_guard<std::mutex>l(auth_mu);std::ofstream f(G.authorized,std::ios::out|std::ios::app);f<<pub<<"|control=1|wake=1|"<<sanitize_label(label)<<"\n";f.close();chmod(G.authorized.c_str(),0600);}
...
bool parse_media_ready(const std::string&line,std::uint32_t generation,StreamOptions&stream,bool&debug){std::istringstream in(line);std::string word,extra;unsigned long long gen=0;int width=0,height=0,fps=0,dbg=0;if(!(in>>word>>gen>>width>>height>>fps>>dbg)||in>>extra||word!="MEDIA_RECEIVER_READY"||gen!=generation)return false;const bool native=width==0&&height==0;const bool bounded=width>=16&&height>=16&&width<=16384&&height<=16384;if((!native&&!bounded)||fps<15||fps>240||(dbg!=0&&dbg!=1))return false;stream={width,height,fps};debug=dbg==1;return true;}
...
bool run_native_session(RendezvousClient&rendezvous,const RendezvousMessage&offer,const std::string&host_public_key){
    ...
    HeldInputState held;VideoSender sender;std::atomic<bool>sender_started{false};PeerSession peer;PeerSession*peer_ptr=&peer;VideoSender*sender_ptr=&sender;const std::uint32_t generation=1;
    PeerSessionOptions options;options.client_side=false;options.socket=socket;options.peer=intro.peer_observed;if(relay_ready)options.relay=PeerRelayFallback{relay.endpoint,relay.allocation_id,RelayRole::Host};options.handshake.rendezvous_id=intro.rendezvous_id;options.handshake.session_id=intro.session_id;options.handshake.generation=generation;options.handshake.client_identity=intro.peer_public_key;options.handshake.host_identity=host_public_key;options.handshake.client_nonce=intro.peer_nonce;options.handshake.host_nonce=intro.local_nonce;options.handshake.auth_binding=was_authorized?"paired":"pairing";options.identity_private_key=G.identity_key;options.pairing_password=was_authorized?"":pairing_password();
    ...
    options.reliable_input=[&](const std::string&line){
        ...
        StreamOptions stream;bool debug=false;if(parse_media_ready(line,generation,stream,debug)){
            if(sender_started.exchange(true))return;const bool audio=host_cfg.get_bool("audio","enabled",true);if(!sender_ptr->start_native(peer_ptr->media_keys(),peer_ptr->session_id(),generation,stream,audio,[peer_ptr](std::span<const std::uint8_t>wire){return peer_ptr->send_media_datagram(wire);},[peer_ptr](const std::string&control){return peer_ptr->send_input(control);})){sender_started.store(false);peer_ptr->send_input("MEDIA_ERROR capture-startup");return;}if(debug)sender_ptr->handle_control_line(debug_media_request_line(generation,true));return;
        }
