...
bool run_peer_session(UdpSocket socket,const RendezvousIntroduction&intro,const std::string&client_public_key,const std::string&host_public_key,bool local_path,std::optional<PeerRelayFallback> relay){
...
bool run_native_session(RendezvousClient&rendezvous,const RendezvousMessage&offer,const std::string&host_public_key){
    RendezvousIntroduction intro;std::string error;if(!rendezvous.accept_offer(offer,host_public_key,G.identity_key,intro,error)){std::cerr<<"OPAL rendezvous accept failed: "<<error<<"\n";return false;}
    RelayAllocation allocation;std::string relay_error;std::optional<PeerRelayFallback> relay;if(rendezvous.request_relay(intro.session_id,host_public_key,G.identity_key,allocation,relay_error))relay=PeerRelayFallback{allocation.endpoint,allocation.allocation_id,RelayRole::Host};auto socket=rendezvous.take_socket();if(socket.fd<0){std::cerr<<"OPAL rendezvous socket unavailable after introduction\n";return false;}return run_peer_session(socket,intro,offer.public_key,host_public_key,false,relay);
}

void local_discovery_loop(const std::string&host_public_key){
    std::string error;auto listener=open_local_discovery_listener(kLocalDiscoveryPort,"0.0.0.0",error);if(listener.fd<0){std::cerr<<"OPAL LAN discovery unavailable: "<<error<<"\n";return;}
    if(debug_enabled())std::cerr<<"OPAL LAN/Tailscale discovery listening UDP "<<listener.local_port<<"\n";
    for(;;){LocalDiscoveryHostResult local;if(!wait_local_client(listener,host_public_key,G.identity_key,local,500,error)){if(error!="local discovery timeout"&&debug_enabled())std::cerr<<"OPAL direct discovery: "<<error<<"\n";continue;}bool expected=false;if(!session_busy.compare_exchange_strong(expected,true)){close_udp_socket(local.socket);continue;}RendezvousIntroduction intro;intro.rendezvous_id=local.rendezvous_id;intro.session_id=local.session_id;intro.peer_public_key=local.client_public_key;intro.local_nonce=local.host_nonce;intro.peer_nonce=local.client_nonce;intro.peer_observed=local.client;run_peer_session(local.socket,intro,local.client_public_key,host_public_key,true,std::nullopt);session_busy.store(false);}
}
