...
bool PeerSession::start(PeerSessionOptions options,std::string&error){stop();impl_=std::make_unique<Impl>();impl_->options=std::move(options);impl_->session_numeric=numeric_session_id(impl_->options.handshake.session_id);if(impl_->options.socket.fd<0||impl_->session_numeric==0||impl_->options.handshake.generation==0||!usable_endpoint(impl_->options.peer)||impl_->options.identity_private_key.empty()){error="invalid peer session options";impl_->set_error(error);return false;}bool connected=false;std::string lan_error,direct_error;if(impl_->options.lan_peer&&usable_endpoint(*impl_->options.lan_peer)&&!same_endpoint(*impl_->options.lan_peer,impl_->options.peer)){if(impl_->attempt(*impl_->options.lan_peer,false,impl_->options.lan_handshake_timeout_ms,lan_error)){impl_->path="lan";connected=true;}}if(!connected){if(impl_->attempt(impl_->options.peer,false,impl_->options.direct_handshake_timeout_ms,direct_error)){impl_->path="direct";connected=true;}else{if(!impl_->options.relay||!retryable_path_error(direct_error)){error=direct_error;if(!lan_error.empty())error="lan: "+lan_error+"; direct: "+direct_error;impl_->set_error(error);return false;}std::string relay_error;if(!impl_->attempt(impl_->options.relay->endpoint,true,impl_->options.relay_handshake_timeout_ms,relay_error)){error="direct: "+direct_error+"; relay: "+relay_error;if(!lan_error.empty())error="lan: "+lan_error+"; "+error;impl_->set_error(error);return false;}impl_->path="relay";connected=true;}}impl_->run.store(true);if(impl_->options.reliable_input)impl_->control_thread=std::thread([this]{impl_->control_loop();});if(impl_->options.pointer_input)impl_->pointer_thread=std::thread([this]{impl_->pointer_loop();});if(impl_->options.media_datagram&&!impl_->options.media_inline)impl_->media_thread=std::thread([this]{impl_->media_loop();});impl_->thread=std::thread([this]{impl_->loop();});error.clear();return true;}
