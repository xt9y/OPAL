...
struct PeerSession::Impl {
    ...
    void reset_attempt(){clear_peer_ephemeral(ephemeral);clear_peer_session_keys(keys);cipher.reset();replay.reset();reliable_sender.reset();reliable_receiver.reset();pointer_receiver.reset();clear_media_queue();clear_control_queue();packet_send_sequence=1;pointer_send_sequence.store(0);reliable_pending_count.store(0);media_drop_count.store(0);media_send_backpressure_count.store(0);control_drop_count.store(0);pointer_overwrite_count.store(0);established.store(false);dropped_test_reliable=false;}
