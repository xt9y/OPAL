...
bool usable_endpoint(const UdpCandidate&e){return !e.host.empty()&&e.port>0;}
...
struct PeerSession::Impl {
    ...
    PeerSessionOptions options;UdpEndpoint peer{};std::uint32_t peer_len=0;std::uint64_t session_numeric=0;
    ...
    std::string pending_pointer;bool pointer_pending=false;
    std::array<std::uint8_t,kSessionPacketHeaderBytes>send_aad{};
    std::array<std::uint8_t,kSessionPacketMaxPayload+16>send_sealed{};
    std::array<std::uint8_t,kSessionPacketMaxBytes>send_wire_buffer{};
    std::array<std::uint8_t,kSessionPacketMaxPayload>receive_plain_buffer{};
    std::atomic<bool>run{false},established{false};std::atomic<std::uint64_t>pointer_send_sequence{0},reliable_pending_count{0},media_drop_count{0},media_send_backpressure_count{0},control_drop_count{0},pointer_overwrite_count{0};std::uint64_t packet_send_sequence=1;std::string error,path="none";bool dropped_test_reliable=false;

    void wake_workers(){media_cv.notify_all();control_cv.notify_all();pointer_cv.notify_all();}
    ...
    void clear_pointer_queue(){std::lock_guard<std::mutex>lock(pointer_mu);pending_pointer.clear();pointer_pending=false;}
    void reset_attempt(){clear_peer_ephemeral(ephemeral);clear_peer_session_keys(keys);cipher.reset();replay.reset();reliable_sender.reset();reliable_receiver.reset();pointer_receiver.reset();clear_media_queue();clear_control_queue();clear_pointer_queue();packet_send_sequence=1;pointer_send_sequence.store(0);reliable_pending_count.store(0);media_drop_count.store(0);media_send_backpressure_count.store(0);control_drop_count.store(0);pointer_overwrite_count.store(0);established.store(false);dropped_test_reliable=false;}
    bool activate(const UdpCandidate&endpoint){peer={};peer_len=0;return usable_endpoint(endpoint)&&resolve_udp_endpoint(endpoint.host,endpoint.port,peer,peer_len);}
    bool send_wire(std::span<const std::uint8_t>wire){return options.socket.fd>=0&&!wire.empty()&&send_datagram(options.socket.fd,peer,peer_len,wire);}
    bool send_media_wire(std::span<const std::uint8_t>wire){if(options.socket.fd<0||wire.empty())return false;const auto result=send_datagram_result(options.socket.fd,peer,peer_len,wire);if(result==UdpSendResult::WouldBlock){media_send_backpressure_count.fetch_add(1,std::memory_order_relaxed);return true;}return result==UdpSendResult::Sent;}
    UdpSendBatchResult send_media_batch(std::span<const std::span<const std::uint8_t>>wires){if(options.socket.fd<0||wires.empty()||wires.size()>kUdpSendBatchMax)return{};for(const auto wire:wires)if(wire.empty()||wire.size()>kVideoMaxDatagramBytes+1)return{};auto result=send_datagrams_batch(options.socket.fd,peer,peer_len,wires);if(result.result==UdpSendResult::WouldBlock&&result.sent<wires.size())media_send_backpressure_count.fetch_add(wires.size()-result.sent,std::memory_order_relaxed);return result;}
    ...
    bool open_encrypted(std::span<const std::uint8_t>wire,SessionPacketHeader*out_header,std::string*out_payload,SessionPacketType require=static_cast<SessionPacketType>(0)){if(!cipher||wire.size()<kSessionPacketHeaderBytes+16)return false;SessionPacketHeader h;if(!parse_session_header(wire,h)||h.generation!=options.handshake.generation||h.session_id!=session_numeric||wire.size()!=kSessionPacketHeaderBytes+h.payload_length+16||h.payload_length>receive_plain_buffer.size())return false;if(static_cast<std::uint8_t>(require)!=0&&h.type!=require)return false;std::size_t plain_size=0;if(!cipher->open(h.packet_sequence,wire.first(kSessionPacketHeaderBytes),wire.subspan(kSessionPacketHeaderBytes),receive_plain_buffer,plain_size)||plain_size!=h.payload_length||!replay.accept(h.packet_sequence))return false;if(out_header)*out_header=h;if(out_payload)out_payload->assign(reinterpret_cast<const char*>(receive_plain_buffer.data()),plain_size);return true;}
    ...
    bool attempt(const UdpCandidate&endpoint,int timeout_ms,std::string&err){reset_attempt();if(!activate(endpoint)){err="Tailscale peer endpoint resolution failed";return false;}return options.client_side?client_handshake(timeout_ms,err):host_handshake(timeout_ms,err);}
    ...
    bool enqueue_control(SessionPacketType type,std::string payload){
        if(type==SessionPacketType::Pointer){
            {std::lock_guard<std::mutex>lock(pointer_mu);if(pointer_pending)pointer_overwrite_count.fetch_add(1);pending_pointer=std::move(payload);pointer_pending=true;}
            pointer_cv.notify_one();return true;
        }
        if(type!=SessionPacketType::ReliableControl)return true;
        {std::lock_guard<std::mutex>lock(control_mu);if(control_count>=control_queue.size()){control_drop_count.fetch_add(1);return false;}const auto tail=(control_head+control_count)%control_queue.size();control_queue[tail].type=type;control_queue[tail].payload=std::move(payload);++control_count;}
    ...
    bool enqueue_media(std::span<const std::uint8_t>wire){if(!options.media_datagram||wire.empty()||wire.size()>kVideoMaxDatagramBytes+1)return false;{std::lock_guard<std::mutex>lock(media_mu);if(media_count>=media_queue.size()){media_queue[media_head].size=0;media_head=(media_head+1)%media_queue.size();--media_count;media_drop_count.fetch_add(1);}const auto tail=(media_head+media_count)%media_queue.size();auto&slot=media_queue[tail];std::copy(wire.begin(),wire.end(),slot.bytes.begin());slot.size=wire.size();++media_count;}media_cv.notify_one();return true;}
    bool dispatch_media(std::span<const std::uint8_t>wire){if(!options.media_datagram||wire.empty()||wire.size()>kVideoMaxDatagramBytes+1)return false;if(options.media_inline){options.media_datagram(wire);return true;}return enqueue_media(wire);}

    void media_loop(){prioritize_low_latency_thread();std::array<std::uint8_t,kVideoMaxDatagramBytes+1>local{};for(;;){std::size_t size=0;{std::unique_lock<std::mutex>lock(media_mu);media_cv.wait_for(lock,std::chrono::milliseconds(20),[&]{return !run.load()||media_count>0;});if(!run.load()&&media_count==0)break;if(media_count==0)continue;auto&slot=media_queue[media_head];size=slot.size;if(size)std::copy_n(slot.bytes.begin(),size,local.begin());slot.size=0;media_head=(media_head+1)%media_queue.size();--media_count;}if(size&&options.media_datagram)options.media_datagram(std::span<const std::uint8_t>(local.data(),size));}}

    void control_loop(){for(;;){ControlSlot work;{std::unique_lock<std::mutex>lock(control_mu);control_cv.wait_for(lock,std::chrono::milliseconds(20),[&]{return !run.load()||control_count>0;});if(!run.load()&&control_count==0)break;if(control_count==0)continue;auto&slot=control_queue[control_head];work.type=slot.type;work.payload=std::move(slot.payload);slot.payload.clear();control_head=(control_head+1)%control_queue.size();--control_count;}if(options.reliable_input)options.reliable_input(work.payload);}}
    void pointer_loop(){prioritize_low_latency_thread();for(;;){std::string work;{std::unique_lock<std::mutex>lock(pointer_mu);pointer_cv.wait_for(lock,std::chrono::milliseconds(20),[&]{return !run.load()||pointer_pending;});if(!run.load())break;if(!pointer_pending)continue;work=std::move(pending_pointer);pending_pointer.clear();pointer_pending=false;}if(options.pointer_input)options.pointer_input(work);}}

    void loop(){prioritize_low_latency_thread();auto next_keepalive=Clock::now()+std::chrono::milliseconds(500),last_authenticated=Clock::now();std::array<std::array<std::uint8_t,kVideoMaxDatagramBytes+1>,kUdpReceiveBatchMax>buffers{};std::array<UdpReceiveSlot,kUdpReceiveBatchMax>slots{};for(std::size_t i=0;i<slots.size();++i)slots[i].buffer=buffers[i];while(run.load()){const int count=recv_datagrams_batch(options.socket.fd,slots,10);if(count>0){for(int i=0;i<count&&run.load();++i){auto&s=slots[static_cast<std::size_t>(i)];if(!same_source(s.source,peer)||s.size==0)continue;const std::span<const std::uint8_t>wire(s.buffer.data(),s.size);if(read_magic(wire)!=kSessionPacketMagic)continue;SessionPacketHeader h;std::string payload;if(open_encrypted(wire,&h,&payload)){last_authenticated=Clock::now();process_control(h,std::move(payload));}}for(int i=0;i<count&&run.load();++i){auto&s=slots[static_cast<std::size_t>(i)];if(!same_source(s.source,peer)||s.size==0)continue;const std::span<const std::uint8_t>wire(s.buffer.data(),s.size);if(read_magic(wire)==kSessionPacketMagic)continue;(void)dispatch_media(wire);}}send_due();const auto now=Clock::now();if(!run.load())break;if(now>=next_keepalive){if(!send_encrypted(SessionPacketType::Keepalive,0,"")){set_error("peer keepalive send failed");run.store(false);wake_workers();break;}next_keepalive=now+std::chrono::milliseconds(500);}if(now-last_authenticated>=std::chrono::seconds(3)){set_error("peer control timed out");run.store(false);wake_workers();break;}}established.store(false);wake_workers();}
};

PeerSession::PeerSession():impl_(std::make_unique<Impl>()){}PeerSession::~PeerSession(){stop();}
bool PeerSession::start(PeerSessionOptions options,std::string&error){stop();impl_=std::make_unique<Impl>();impl_->options=std::move(options);impl_->session_numeric=numeric_session_id(impl_->options.handshake.session_id);if(impl_->options.socket.fd<0||impl_->session_numeric==0||impl_->options.handshake.generation==0||!usable_endpoint(impl_->options.peer)||impl_->options.identity_private_key.empty()){error="invalid peer session options";impl_->set_error(error);return false;}if(!impl_->attempt(impl_->options.peer,impl_->options.handshake_timeout_ms,error)){impl_->set_error(error);return false;}impl_->path="tailnet-direct";impl_->run.store(true);if(impl_->options.reliable_input)impl_->control_thread=std::thread([this]{impl_->control_loop();});if(impl_->options.pointer_input)impl_->pointer_thread=std::thread([this]{impl_->pointer_loop();});if(impl_->options.media_datagram&&!impl_->options.media_inline)impl_->media_thread=std::thread([this]{impl_->media_loop();});impl_->thread=std::thread([this]{impl_->loop();});error.clear();return true;}
...
bool PeerSession::send_media_datagram(std::span<const std::uint8_t>wire){return impl_&&impl_->run.load()&&wire.size()<=kVideoMaxDatagramBytes+1&&impl_->send_media_wire(wire);}
UdpSendBatchResult PeerSession::send_media_datagrams(std::span<const std::span<const std::uint8_t>>wires){if(!impl_||!impl_->run.load()||wires.empty()||wires.size()>kUdpSendBatchMax)return{};for(const auto wire:wires)if(wire.empty()||wire.size()>kVideoMaxDatagramBytes+1)return{};return impl_->send_media_batch(wires);}
...
std::uint64_t PeerSession::media_send_backpressure_drops()const{return impl_?impl_->media_send_backpressure_count.load():0;}
...
void PeerSession::stop(){if(!impl_)return;impl_->run.store(false);impl_->wake_workers();if(impl_->thread.joinable())impl_->thread.join();if(impl_->media_thread.joinable())impl_->media_thread.join();if(impl_->control_thread.joinable())impl_->control_thread.join();if(impl_->pointer_thread.joinable())impl_->pointer_thread.join();impl_->clear_media_queue();impl_->clear_control_queue();impl_->clear_pointer_queue();impl_->established.store(false);close_udp_socket(impl_->options.socket);impl_->cipher.reset();clear_peer_ephemeral(impl_->ephemeral);clear_peer_session_keys(impl_->keys);}
