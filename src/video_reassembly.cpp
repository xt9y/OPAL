...
struct VideoReassembler::Impl{
    ...
    std::uint64_t session_id=0,order=0,video_order=0,last_video_id=0;
    ...
    void release(Frame&frame){if(!frame.active)return;if(bytes>=frame.storage_bytes)bytes-=frame.storage_bytes;else bytes=0;frame.deactivate();}
...
void VideoReassembler::reset(std::uint32_t generation,std::uint64_t session_id){impl_->generation=generation;impl_->session_id=session_id;impl_->active=true;impl_->clear_active();impl_->order=0;impl_->video_order=0;impl_->last_video_id=0;impl_->awaiting_idr=true;}
void VideoReassembler::require_idr(){if(!impl_)return;impl_->awaiting_idr=true;for(auto&frame:impl_->frames)if(frame.active&&frame.type_known&&frame.type==VideoMediaType::VideoH264)impl_->release(frame);}
...
ReassemblyStatus VideoReassembler::accept(const VideoPacketHeader&header,std::span<const std::uint8_t>payload,ReassembledFrame&output){
    ...
    bool need_idr=false;auto*frame=impl.find(header.frame_id);
    if(!frame){
        const std::size_t planned=storage_for(header.fragment_count);if(!planned)return ReassemblyStatus::Ignored;
        while(impl.count()>=kFrameSlots||impl.bytes+planned>kMaxBytes){auto*victim=impl.oldest();if(!victim)break;if(victim->type_known&&victim->type==VideoMediaType::VideoH264){need_idr=true;impl.awaiting_idr=true;}impl.release(*victim);}
        if(impl.bytes+planned>kMaxBytes)return need_idr?ReassemblyStatus::NeedIdr:ReassemblyStatus::Ignored;
        frame=impl.free_slot();if(!frame)return need_idr?ReassemblyStatus::NeedIdr:ReassemblyStatus::Ignored;
    ...
    if(header.media_type==VideoMediaType::Fec){
        ...
        if(frame->type==VideoMediaType::VideoH264&&(frame->flags&FrameConfig)==0&&frame->video_order==0){
            ...
            for(auto&older:impl.frames){if(!older.active||&older==frame)continue;if(older.type_known&&older.type==VideoMediaType::VideoH264&&older.video_order>0&&frame->video_order>=older.video_order+2){impl.release(older);need_idr=true;impl.awaiting_idr=true;}}
        }
        if(frame->type==VideoMediaType::VideoH264&&impl.awaiting_idr&&(frame->flags&(FrameKeyframe|FrameConfig))==0){impl.release(*frame);return ReassemblyStatus::NeedIdr;}
    ...
    if(frame->type==VideoMediaType::VideoH264){
        if((frame->flags&(FrameKeyframe|FrameConfig))==0){
            bool older_pending=false;for(const auto&entry:impl.frames)if(entry.active&&entry.id<frame->id&&entry.type_known&&entry.type==VideoMediaType::VideoH264){older_pending=true;break;}
            if(older_pending){impl.awaiting_idr=true;const auto completed_id=frame->id;for(auto&entry:impl.frames)if(entry.active&&entry.id<=completed_id&&entry.type_known&&entry.type==VideoMediaType::VideoH264)impl.release(entry);return ReassemblyStatus::NeedIdr;}
        }else if((frame->flags&FrameKeyframe)!=0){
            const auto key_id=frame->id;for(auto&entry:impl.frames)if(entry.active&&&entry!=frame&&entry.id<key_id&&entry.type_known&&entry.type==VideoMediaType::VideoH264)impl.release(entry);
        }
    }
