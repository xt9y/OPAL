...
void VideoCapture::stop(){if(!impl_)return;if(impl_->packet)av_packet_unref(impl_->packet);if(impl_->format){avformat_close_input(&impl_->format);impl_->format=nullptr;}if(impl_->avio){av_freep(&impl_->avio->buffer);avio_context_free(&impl_->avio);impl_->avio=nullptr;}stop_capture(impl_->capture);impl_->video_stream=-1;impl_->audio_stream=-1;impl_->terminal=false;impl_->timeline_anchored=false;impl_->timeline_origin_pts_us=0;impl_->timeline_origin_monotonic_us=0;impl_->configs.clear();}
}
