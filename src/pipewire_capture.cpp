...
std::vector<std::uint8_t> annexb_parameter_sets(std::span<const std::uint8_t>data){
    std::vector<std::uint8_t>out;bool sps=false,pps=false;std::size_t pos=0;
    auto start_code=[&](std::size_t from,std::size_t&begin,std::size_t&bytes){for(std::size_t i=from;i+3<=data.size();++i){if(data[i]!=0||data[i+1]!=0)continue;if(data[i+2]==1){begin=i;bytes=3;return true;}if(i+4<=data.size()&&data[i+2]==0&&data[i+3]==1){begin=i;bytes=4;return true;}}return false;};
    for(;;){std::size_t begin=0,prefix=0;if(!start_code(pos,begin,prefix))break;const std::size_t nal=begin+prefix;if(nal>=data.size())break;std::size_t next=0,next_prefix=0;const bool have_next=start_code(nal+1,next,next_prefix);const std::size_t end=have_next?next:data.size();const auto type=data[nal]&0x1f;if(type==7||type==8){out.insert(out.end(),data.begin()+static_cast<std::ptrdiff_t>(begin),data.begin()+static_cast<std::ptrdiff_t>(end));sps|=type==7;pps|=type==8;}if(!have_next)break;pos=next;(void)next_prefix;}
    if(!sps||!pps)out.clear();return out;
}
}
...
struct NativePipeWireVideoCapture::Impl {
...
#if OPAL_HAVE_NATIVE_PIPEWIRE
    struct RawFrame {std::vector<std::uint8_t>pixels;int width=0,height=0,stride=0;AVPixelFormat format=AV_PIX_FMT_NONE;std::uint64_t capture_us=0;bool source_dmabuf=false;};
    struct EncoderState {
        AVCodecContext*ctx=nullptr;AVFrame*sw_frame=nullptr;AVFrame*hw_frame=nullptr;AVPacket*packet=nullptr;SwsContext*sws=nullptr;AVBufferRef*hw_device=nullptr;bool hardware=false;AVPixelFormat convert_format=AV_PIX_FMT_YUV420P;std::string name;
    ...
    std::condition_variable raw_cv;std::deque<RawFrame>raw_frames;std::thread encoder_thread;std::mutex portal_mu;GCancellable*cancellable=nullptr;spa_video_info_raw raw_info{};int bitrate_kbps=0,fps=60,preferred_width=1920,preferred_height=1080;std::string token_file;pw_stream*stream=nullptr;std::atomic<bool>saw_dmabuf{false};
    ...
    static void stream_state_changed(void*data,pw_stream_state,pw_stream_state state,const char*message){auto*self=static_cast<Impl*>(data);if(state!=PW_STREAM_STATE_ERROR&&state!=PW_STREAM_STATE_UNCONNECTED)return;if(self->run.load())self->set_error(message&&*message?message:"PipeWire stream disconnected");}
    static void stream_param_changed(void*data,std::uint32_t id,const spa_pod*param){
        auto*self=static_cast<Impl*>(data);if(id!=SPA_PARAM_Format||!param)return;spa_video_info_raw info{};if(spa_format_video_raw_parse(param,&info)<0)return;self->raw_info=info;if(!self->stream||!info.size.width||!info.size.height)return;
        const int stride=static_cast<int>(info.size.width)*4;const int size=stride*static_cast<int>(info.size.height);std::array<std::uint8_t,768>storage{};spa_pod_builder builder=SPA_POD_BUILDER_INIT(storage.data(),storage.size());const spa_pod*params[2];params[0]=static_cast<const spa_pod*>(spa_pod_builder_add_object(&builder,SPA_TYPE_OBJECT_ParamBuffers,SPA_PARAM_Buffers,SPA_PARAM_BUFFERS_buffers,SPA_POD_CHOICE_RANGE_Int(4,2,8),SPA_PARAM_BUFFERS_blocks,SPA_POD_Int(1),SPA_PARAM_BUFFERS_size,SPA_POD_Int(size),SPA_PARAM_BUFFERS_stride,SPA_POD_Int(stride),SPA_PARAM_BUFFERS_dataType,SPA_POD_CHOICE_FLAGS_Int((1<<SPA_DATA_DmaBuf)|(1<<SPA_DATA_MemFd)|(1<<SPA_DATA_MemPtr))));params[1]=static_cast<const spa_pod*>(spa_pod_builder_add_object(&builder,SPA_TYPE_OBJECT_ParamMeta,SPA_PARAM_Meta,SPA_PARAM_META_type,SPA_POD_Id(SPA_META_Header),SPA_PARAM_META_size,SPA_POD_Int(static_cast<int>(sizeof(spa_meta_header)))));(void)pw_stream_update_params(self->stream,params,2);
    }
    ...
    std::uint64_t capture_cycle_us(const pw_buffer*buffer)const{const auto local_now=monotonic_us();if(!stream||!buffer)return local_now;std::uint64_t source_time=buffer->time;if(buffer->buffer){const auto*header=static_cast<const spa_meta_header*>(spa_buffer_find_meta_data(buffer->buffer,SPA_META_Header,sizeof(spa_meta_header)));if(header&&header->pts!=SPA_TIME_INVALID&&header->pts>=0)source_time=static_cast<std::uint64_t>(header->pts);}if(!source_time)return local_now;const auto pipewire_now=pw_stream_get_nsec(stream);if(pipewire_now<source_time)return local_now;const auto age_us=(pipewire_now-source_time)/SPA_NSEC_PER_USEC;if(age_us>5000000u||age_us>local_now)return local_now;return local_now-age_us;}

    void capture_latest_buffer(){
        if(!stream||!run.load())return;pw_buffer*selected=nullptr;for(;;){pw_buffer*next=pw_stream_dequeue_buffer(stream);if(!next)break;if(selected)pw_stream_queue_buffer(stream,selected);selected=next;}if(!selected)return;auto requeue=[&]{pw_stream_queue_buffer(stream,selected);};spa_buffer*buffer=selected->buffer;if(!buffer||buffer->n_datas==0||raw_info.size.width==0||raw_info.size.height==0){requeue();return;}spa_data&d=buffer->datas[0];if(!d.chunk||d.chunk->stride<=0){requeue();return;}const AVPixelFormat format=av_format(raw_info.format);if(format==AV_PIX_FMT_NONE){requeue();return;}const int width=static_cast<int>(raw_info.size.width),height=static_cast<int>(raw_info.size.height),source_stride=d.chunk->stride;const std::size_t row_bytes=static_cast<std::size_t>(width)*4u;if(source_stride<static_cast<int>(row_bytes)){requeue();return;}const std::uint64_t end=static_cast<std::uint64_t>(d.chunk->offset)+static_cast<std::uint64_t>(height-1)*static_cast<std::uint64_t>(source_stride)+row_bytes;if(end>d.maxsize){requeue();return;}
        ...
        RawFrame frame;{std::lock_guard<std::mutex>lock(mu);if(raw_frames.size()>=2){frame.pixels=std::move(raw_frames.front().pixels);raw_frames.pop_front();}}frame.pixels.resize(row_bytes*static_cast<std::size_t>(height));frame.width=width;frame.height=height;frame.stride=static_cast<int>(row_bytes);frame.format=format;frame.capture_us=capture_cycle_us(selected);frame.source_dmabuf=d.type==SPA_DATA_DmaBuf;const auto*src=base+d.chunk->offset;for(int y=0;y<height;++y)std::copy_n(src+static_cast<std::ptrdiff_t>(y)*source_stride,row_bytes,frame.pixels.data()+static_cast<std::size_t>(y)*row_bytes);if(mapping)munmap(mapping,d.maxsize);requeue();if(frame.source_dmabuf)saw_dmabuf.store(true);{std::lock_guard<std::mutex>lock(mu);if(raw_frames.size()>=2)raw_frames.pop_front();raw_frames.push_back(std::move(frame));}raw_cv.notify_one();
    }
    ...
    bool open_vaapi(EncoderState&enc,const RawFrame&raw){const AVCodec*codec=avcodec_find_encoder_by_name("h264_vaapi");if(!codec)return false;if(av_hwdevice_ctx_create(&enc.hw_device,AV_HWDEVICE_TYPE_VAAPI,nullptr,nullptr,0)<0||!enc.hw_device){enc.reset();return false;}enc.ctx=avcodec_alloc_context3(codec);if(!enc.ctx){enc.reset();return false;}enc.convert_format=AV_PIX_FMT_NV12;configure_common(enc.ctx,raw,AV_PIX_FMT_VAAPI);enc.ctx->slices=low_latency_h264_slices(raw.width,raw.height);AVBufferRef*frames_ref=av_hwframe_ctx_alloc(enc.hw_device);if(!frames_ref){enc.reset();return false;}auto*frames=reinterpret_cast<AVHWFramesContext*>(frames_ref->data);frames->format=AV_PIX_FMT_VAAPI;frames->sw_format=AV_PIX_FMT_NV12;frames->width=raw.width;frames->height=raw.height;frames->initial_pool_size=4;if(av_hwframe_ctx_init(frames_ref)<0){av_buffer_unref(&frames_ref);enc.reset();return false;}enc.ctx->hw_frames_ctx=av_buffer_ref(frames_ref);av_buffer_unref(&frames_ref);if(!enc.ctx->hw_frames_ctx){enc.reset();return false;}AVDictionary*options=nullptr;av_dict_set(&options,"async_depth","1",0);const int rc=avcodec_open2(enc.ctx,codec,&options);av_dict_free(&options);if(rc<0){enc.reset();return false;}enc.hw_frame=av_frame_alloc();if(!enc.hw_frame||!allocate_sw_frame(enc,raw)){enc.reset();return false;}enc.hardware=true;enc.name="h264_vaapi";return true;}
    static bool accepts_cpu_format(const AVCodec*codec,AVPixelFormat&format){if(!codec)return false;const AVPixelFormat*formats=nullptr;
...
#endif
        return false;}
    bool open_cpu_input_encoder(EncoderState&enc,const RawFrame&raw){constexpr const char*candidates[]={"h264_v4l2m2m","h264_nvenc","h264_qsv","libx264","libopenh264"};for(const char*name:candidates){const AVCodec*codec=avcodec_find_encoder_by_name(name);AVPixelFormat format=AV_PIX_FMT_NONE;if(!accepts_cpu_format(codec,format))continue;enc.ctx=avcodec_alloc_context3(codec);if(!enc.ctx)continue;enc.convert_format=format;configure_common(enc.ctx,raw,format);AVDictionary*options=nullptr;const std::string encoder_name=name;if(encoder_name=="h264_nvenc"){av_dict_set(&options,"preset","p1",0);av_dict_set(&options,"tune","ull",0);av_dict_set(&options,"zerolatency","1",0);av_dict_set(&options,"delay","0",0);}else if(encoder_name=="h264_qsv"){av_dict_set(&options,"async_depth","1",0);av_dict_set(&options,"preset","veryfast",0);}else if(encoder_name=="libx264"){enc.ctx->slices=low_latency_h264_slices(raw.width,raw.height);av_dict_set(&options,"preset","ultrafast",0);av_dict_set(&options,"tune","zerolatency",0);}const int rc=avcodec_open2(enc.ctx,codec,&options);av_dict_free(&options);if(rc<0){enc.reset();continue;}if(!allocate_sw_frame(enc,raw)){enc.reset();continue;}enc.hardware=encoder_name!="libx264"&&encoder_name!="libopenh264";enc.name=name;return true;}return false;}
    void publish_config(AVCodecContext*ctx,std::span<const std::uint8_t>packet,bool keyframe){if(!config.extradata.empty())return;std::vector<std::uint8_t>extra;if(ctx&&ctx->extradata&&ctx->extradata_size>0)extra.assign(ctx->extradata,ctx->extradata+ctx->extradata_size);else if(keyframe)extra=annexb_parameter_sets(packet);if(extra.empty())return;config.kind=MediaKind::VideoH264;config.extradata=std::move(extra);++config_rev;}
    bool configure_encoder(EncoderState&enc,const RawFrame&raw){enc.reset();if(!open_vaapi(enc,raw)&&!open_cpu_input_encoder(enc,raw)){set_error("no usable in-process low-latency H.264 encoder");return false;}{std::lock_guard<std::mutex>lock(mu);backend=std::string("pipewire-native+")+enc.name+(enc.hardware?"+hw":"+sw")+(saw_dmabuf.load()?"+dmabuf-source":"");publish_config(enc.ctx,{},false);}return true;}
    bool encode_one(EncoderState&enc,const RawFrame&raw,std::uint64_t frame_index){if(av_frame_make_writable(enc.sw_frame)<0)return false;const std::uint8_t*src[4]={raw.pixels.data(),nullptr,nullptr,nullptr};int src_stride[4]={raw.stride,0,0,0};if(sws_scale(enc.sws,src,src_stride,0,raw.height,enc.sw_frame->data,enc.sw_frame->linesize)<=0)return false;enc.sw_frame->pts=static_cast<std::int64_t>(frame_index);AVFrame*submit=enc.sw_frame;if(enc.ctx->pix_fmt==AV_PIX_FMT_VAAPI){av_frame_unref(enc.hw_frame);if(av_hwframe_get_buffer(enc.ctx->hw_frames_ctx,enc.hw_frame,0)<0||av_hwframe_transfer_data(enc.hw_frame,enc.sw_frame,0)<0)return false;enc.hw_frame->pts=enc.sw_frame->pts;submit=enc.hw_frame;}if(avcodec_send_frame(enc.ctx,submit)<0)return false;for(;;){const int rc=avcodec_receive_packet(enc.ctx,enc.packet);if(rc==AVERROR(EAGAIN)||rc==AVERROR_EOF)break;if(rc<0)return false;const bool keyframe=(enc.packet->flags&AV_PKT_FLAG_KEY)!=0;{std::lock_guard<std::mutex>lock(mu);publish_config(enc.ctx,std::span<const std::uint8_t>(enc.packet->data,enc.packet->size),keyframe);}EncodedMediaUnit unit;unit.kind=MediaKind::VideoH264;unit.data.assign(enc.packet->data,enc.packet->data+enc.packet->size);unit.pts_us=static_cast<std::int64_t>(raw.capture_us);unit.capture_time_us=raw.capture_us;unit.keyframe=keyframe;av_packet_unref(enc.packet);{std::lock_guard<std::mutex>lock(mu);if(encoded.size()>=2)encoded.pop_front();encoded.push_back(std::move(unit));}encoded_cv.notify_one();}return true;}
