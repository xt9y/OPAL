...
struct NativePipeWireVideoCapture::Impl {
    ...
    void capture_latest_buffer(){
        ...
        frame.pixels.resize(row_bytes*static_cast<std::size_t>(height));frame.width=width;frame.height=height;frame.stride=static_cast<int>(row_bytes);frame.format=format;frame.capture_us=selected->time?selected->time/1000u:monotonic_us();frame.source_dmabuf=d.type==SPA_DATA_DmaBuf;
    ...
    static bool accepts_cpu_format(const AVCodec*codec,AVPixelFormat&format){if(!codec)return false;if(!codec->pix_fmts){format=AV_PIX_FMT_YUV420P;return true;}for(const AVPixelFormat*p=codec->pix_fmts;*p!=AV_PIX_FMT_NONE;++p)if(*p==AV_PIX_FMT_NV12){format=*p;return true;}for(const AVPixelFormat*p=codec->pix_fmts;*p!=AV_PIX_FMT_NONE;++p)if(*p==AV_PIX_FMT_YUV420P){format=*p;return true;}return false;}
    ...
    void setup_loop(){
        ...
        std::array<std::uint8_t,1024>pod_buffer{};spa_pod_builder builder=SPA_POD_BUILDER_INIT(pod_buffer.data(),pod_buffer.size());const spa_pod*params[1];params[0]=spa_pod_builder_add_object(&builder,SPA_TYPE_OBJECT_Format,SPA_PARAM_EnumFormat,SPA_FORMAT_mediaType,SPA_POD_Id(SPA_MEDIA_TYPE_video),SPA_FORMAT_mediaSubtype,SPA_POD_Id(SPA_MEDIA_SUBTYPE_raw),SPA_FORMAT_VIDEO_format,SPA_POD_CHOICE_ENUM_Id(5,SPA_VIDEO_FORMAT_BGRA,SPA_VIDEO_FORMAT_BGRA,SPA_VIDEO_FORMAT_BGRx,SPA_VIDEO_FORMAT_RGBA,SPA_VIDEO_FORMAT_RGBx),SPA_FORMAT_VIDEO_size,SPA_POD_CHOICE_RANGE_Rectangle(&SPA_RECTANGLE(preferred_width,preferred_height),&SPA_RECTANGLE(16,16),&SPA_RECTANGLE(7680,4320)),SPA_FORMAT_VIDEO_framerate,SPA_POD_CHOICE_RANGE_Fraction(&SPA_FRACTION(fps,1),&SPA_FRACTION(1,1),&SPA_FRACTION(240,1)));
