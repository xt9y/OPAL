...
namespace opal {
namespace {
constexpr std::uint32_t kAudioBaseQueueMs=24;
constexpr std::uint32_t kAudioMaxQueueMs=40;
constexpr std::uint32_t kAudioPulseTargetMs=20;
constexpr std::uint32_t kAudioPulseMinRequestMs=5;
}

struct AudioOutput::Impl {
    using Clock=std::chrono::steady_clock;
    struct Chunk {std::vector<std::uint8_t> pcm;std::uint32_t ms=0;std::int64_t pts_us=0;};
    AVCodecContext *decoder=nullptr;SwrContext *swr=nullptr;AVPacket *packet=nullptr;AVFrame *frame=nullptr;AVChannelLayout swr_input_layout{};bool swr_layout_valid=false;AVSampleFormat swr_input_format=AV_SAMPLE_FMT_NONE;int swr_input_rate=0;int sample_rate=0,channels=0;std::vector<std::uint8_t> packet_bytes,pcm_scratch;mutable std::mutex mutex;std::condition_variable cv;std::deque<Chunk> queue;std::uint32_t queue_ms=0,target_queue_ms=kAudioBaseQueueMs,stable_enqueues=0;bool run=false;std::thread thread;std::string test_sink;Clock::time_point last_dequeue{};
    ~Impl(){release_codec_state();}
    void clear_locked(){queue.clear();queue_ms=0;last_dequeue={};stable_enqueues=0;target_queue_ms=kAudioBaseQueueMs;}
    ...
    void enqueue(Chunk chunk){
        ...
        if(chunk.ms>kAudioMaxQueueMs){const std::size_t bytes_per_ms=std::max<std::size_t>(1,static_cast<std::size_t>(sample_rate)*channels*2/1000),keep=std::min(chunk.pcm.size(),bytes_per_ms*kAudioMaxQueueMs);std::vector<std::uint8_t> tail(chunk.pcm.end()-static_cast<std::ptrdiff_t>(keep),chunk.pcm.end());chunk.pcm=std::move(tail);chunk.ms=kAudioMaxQueueMs;}
        std::lock_guard<std::mutex> lock(mutex);
        const auto now=Clock::now();
        const bool starved=queue.empty()&&last_dequeue.time_since_epoch().count()!=0&&now-last_dequeue>std::chrono::milliseconds(35);
        if(starved){target_queue_ms=std::min(kAudioMaxQueueMs,target_queue_ms+4);stable_enqueues=0;}
        else if(++stable_enqueues>=200&&target_queue_ms>kAudioBaseQueueMs){target_queue_ms=std::max(kAudioBaseQueueMs,target_queue_ms-2);stable_enqueues=0;}
        while(!queue.empty()&&queue_ms+chunk.ms>target_queue_ms){queue_ms-=queue.front().ms;queue.pop_front();}
        if(queue_ms+chunk.ms>target_queue_ms)return;
    ...
    void playback(){
        ...
        if(test_sink.empty()){
            ...
            pa_buffer_attr attr{};attr.maxlength=bytes_per_ms*kAudioMaxQueueMs;attr.tlength=bytes_per_ms*kAudioPulseTargetMs;attr.prebuf=0;attr.minreq=bytes_per_ms*kAudioPulseMinRequestMs;attr.fragsize=static_cast<std::uint32_t>(-1);
            int error=0;pulse=pa_simple_new(nullptr,"OPAL",PA_STREAM_PLAYBACK,nullptr,"Remote audio",&spec,nullptr,&attr,&error);
        }
        for(;;){Chunk chunk;{std::unique_lock<std::mutex> lock(mutex);cv.wait(lock,[&]{return !run||!queue.empty();});if(!run){clear_locked();break;}if(test_sink=="hold"){lock.unlock();std::this_thread::sleep_for(std::chrono::milliseconds(5));continue;}chunk=std::move(queue.front());queue.pop_front();queue_ms-=chunk.ms;last_dequeue=Clock::now();}if(test_sink=="discard"||!pulse)continue;int error=0;if(pa_simple_write(pulse,chunk.pcm.data(),chunk.pcm.size(),&error)<0){pa_simple_free(pulse);pulse=nullptr;}}
