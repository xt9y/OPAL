...
int socket_fd(const UdpSocket& socket){return socket.valid()?static_cast<int>(socket.handle):-1;}
...
bool rendezvous_endpoint(const UdpEndpoint& source,RendezvousEndpoint& endpoint){
    std::string host;std::uint16_t port=0;if(!udp_endpoint_numeric(source,host,port))return false;endpoint={std::move(host),port};return true;
}
...
bool wait_local_client(UdpSocket &listener,const std::string &host_public_key,
    ...
    result={};error.clear();const auto id=rendezvous_id_from_public_key(host_public_key);if(!listener.valid()||listener.local_port==0||id.empty()||!valid_public_key(host_public_key)){error="local discovery listener unavailable";return false;}
    ...
    while(remaining_ms(deadline)>0){UdpEndpoint source{};const int n=recv_datagram(listener,buffer,source,remaining_ms(deadline));if(n==-2)continue;if(n<0){error="local discovery receive failed";return false;}if(n<=0||n>static_cast<int>(buffer.size()))continue;
        ...
        RendezvousEndpoint client;if(!rendezvous_endpoint(source,client))continue;
        if(client.port!=client_peer_port)continue;
        const int peer_fd=fcntl(socket_fd(listener),F_DUPFD_CLOEXEC,0);if(peer_fd<0){error="local peer socket duplication failed";return false;}UdpSocket peer_socket{peer_fd,listener.local_port};
...
bool discover_local_host(const std::string &rendezvous_id,const std::string &client_public_key,
    ...
    auto peer_socket=open_udp_socket();if(!peer_socket.valid()){error="local discovery socket failed";return false;}
    int one=1;if(setsockopt(socket_fd(peer_socket),SOL_SOCKET,SO_BROADCAST,&one,sizeof(one))!=0){close_udp_socket(peer_socket);error="local discovery broadcast unavailable";return false;}
    UdpEndpoint target{};if(!resolve_udp_endpoint(destination_host,destination_port,target)){close_udp_socket(peer_socket);error="local discovery destination unavailable";return false;}
    ...
    while(remaining_ms(deadline)>0){const auto now=Clock::now();if(next_send.time_since_epoch().count()==0||now>=next_send){(void)send_datagram(peer_socket,target,std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(request.data()),request.size()));next_send=now+std::chrono::milliseconds(75);}const int wait_ms=std::min(75,remaining_ms(deadline));UdpEndpoint source{};const int n=recv_datagram(peer_socket,buffer,source,wait_ms);if(n==-2)continue;if(n<0){close_udp_socket(peer_socket);error="local discovery receive failed";return false;}if(n<=0||n>static_cast<int>(buffer.size()))continue;
        ...
        RendezvousEndpoint host;if(!rendezvous_endpoint(source,host)){rejection_error="local discovery offer source invalid";continue;}host.port=host_peer_port;
