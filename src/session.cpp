...
struct SessionSupervisor::Impl {
    ...
    void control_loop(){
        ...
        while(run.load()){unsigned long g=generation.load();std::string command;{std::lock_guard<std::mutex>l(queue_mu);if(!outbound.empty()){command=std::move(outbound.front());outbound.pop_front();}}
            ...
            bool got_input=false;int fd=current_control_fd();if(fd>=0){{std::lock_guard<std::mutex>l(control_mu);if(control.ssl)got_input=SSL_pending(control.ssl)>0;}if(!got_input&&command.empty())got_input=wait_for_input(fd,20);}
