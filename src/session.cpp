...
struct SessionSupervisor::Impl {
    ...
    bool authenticate(TlsConn&c,bool allow_pair,std::string&token,std::string&fp){if(!verify_fingerprint(c,fp))return false;std::string challenge;if(!tls_read_line_timeout(c.ssl,challenge,5000)||challenge.rfind("CHALLENGE ",0)!=0){set_error("control authentication challenge failed");return false;}auto payload=challenge.substr(10);if(payload.rfind("OPAL3 ",0)!=0){set_error("incompatible OPAL host protocol; update OPAL on both computers");return false;}auto nonce=payload.substr(6);int pw=0,ph=0;std::string pmac;{std::istringstream m(nonce);std::string random,mac,extra;int w=0,h=0;if(m>>random>>w>>h){if(w>0&&h>0){pw=w;ph=h;}if(m>>mac){if(!(m>>extra)&&valid_mac(mac))pmac=mac;else{pw=0;ph=0;pmac.clear();}}}}std::string auth;if(paired_state.load()){if(options.client_public_key.empty()||options.client_private_key_path.empty()){set_error("saved client identity unavailable");return false;}auto sig=sign_hex(std::filesystem::path(options.client_private_key_path),nonce);if(sig.empty()){set_error("client authentication signature failed");return false;}auth="AUTH "+options.client_public_key+" "+sig;}else{if(!allow_pair){set_error("paired recovery cannot fall back to pairing");return false;}std::string password=options.pairing_password;if(password.empty()&&options.pairing_password_provider)password=options.pairing_password_provider();password=normalize_pairing_code(password);if(password.empty()){set_error("pairing password required");return false;}auto transcript="OPAL-PAIR-v3\n"+fp+"\n"+nonce+"\n"+options.client_public_key;auth="PAIR "+options.client_public_key+" "+hmac_sha256_hex(password,transcript);if(!options.label.empty())auth+=" "+options.label;}if(!tls_write_line_timeout(c.ssl,auth,2000)){set_error("control authentication write failed");return false;}std::string reply;if(!tls_read_line_timeout(c.ssl,reply,5000)){set_error("control authentication reply failed");return false;}if(reply.rfind("OK ",0)!=0){set_error(paired_state.load()?"saved client authentication denied":"pairing password incorrect or expired");return false;}token=reply.substr(3);if(token.empty()){set_error("host returned an empty session token");return false;}if((pw>0&&ph>0)||!pmac.empty()){std::lock_guard<std::mutex>l(state_mu);if(pw>0&&ph>0){remote_width_value=pw;remote_height_value=ph;}if(!pmac.empty())remote_mac_value=pmac;}paired_state.store(true);return true;}
    ...
    bool negotiate_media(TlsConn &c,const std::string &token,const std::string &fp,unsigned long g){
        ...
        std::string negotiation_error;auto send=[&](const std::string &line,int timeout){return tls_write_line_timeout(c.ssl,line,timeout);};auto read=[&](std::string &line,int timeout){return tls_read_line_timeout(c.ssl,line,timeout);};DirectVideoPath path;
    ...
    bool recover_control(unsigned long failed,bool video_stall=false){std::lock_guard<std::mutex>r(recovery_mu);if(!run.load())return false;if(generation.load()!=failed)return true;std::cout<<(video_stall?"Direct video stalled; recovering direct session...\n":"Control interrupted; recovering direct session...\n")<<std::flush;stop_receiver();clear_queue();int fd=control_fd.exchange(-1);if(fd>=0)shutdown(fd,SHUT_RDWR);{std::lock_guard<std::mutex>l(control_mu);close_tls(control);}if(!ensure_tunnel()){run.store(false);queue_cv.notify_all();return false;}TlsConn next;std::string token,fp;if(!connect_authenticated(false,next,token,fp)){run.store(false);queue_cv.notify_all();return false;}const unsigned long next_generation=failed+1;if(!negotiate_media(next,token,fp,next_generation)){close_tls(next);run.store(false);queue_cv.notify_all();return false;}install_control(next,fp,next_generation);std::cout<<(video_stall?"Direct video restored with fresh keys.\n":"Control restored. Direct video rekeyed.\n")<<std::flush;return true;}
    ...
    void control_loop(){auto next_ping=std::chrono::steady_clock::now()+2s;bool awaiting_pong=false;auto pong_deadline=next_ping;while(run.load()){unsigned long g=generation.load();if(receiver_failed()){set_error("direct video path timed out");if(!recover_control(g,true))break;next_ping=std::chrono::steady_clock::now()+2s;awaiting_pong=false;continue;}std::string command;{std::lock_guard<std::mutex>l(queue_mu);if(!outbound.empty()){command=std::move(outbound.front());outbound.pop_front();}}if(!command.empty()&&!write_control(command,250)){if(!recover_control(g))break;next_ping=std::chrono::steady_clock::now()+2s;awaiting_pong=false;continue;}bool got_input=false;int fd=current_control_fd();if(fd>=0){{std::lock_guard<std::mutex>l(control_mu);if(control.ssl)got_input=tls_line_ready(control.ssl)||SSL_pending(control.ssl)>0;}if(!got_input&&command.empty())got_input=wait_for_input(fd,20);}if(got_input){std::string line;if(!read_control(line,250)){if(!recover_control(g))break;next_ping=std::chrono::steady_clock::now()+2s;awaiting_pong=false;continue;}if(line=="PONG")awaiting_pong=false;else if(!dispatch_control_line(line)){set_error("unexpected OPAL control message");if(!recover_control(g))break;next_ping=std::chrono::steady_clock::now()+2s;awaiting_pong=false;continue;}}auto now=std::chrono::steady_clock::now();if(awaiting_pong&&now>=pong_deadline){if(!recover_control(g))break;next_ping=std::chrono::steady_clock::now()+2s;awaiting_pong=false;continue;}if(!awaiting_pong&&now>=next_ping){if(!write_control("PING",500)){if(!recover_control(g))break;next_ping=std::chrono::steady_clock::now()+2s;continue;}awaiting_pong=true;pong_deadline=now+1500ms;next_ping=now+2s;}if(command.empty()&&!got_input){std::unique_lock<std::mutex>l(queue_mu);queue_cv.wait_for(l,5ms,[&]{return !run.load()||!outbound.empty();});}}}
    bool start(){if(run.load())return true;signal(SIGPIPE,SIG_IGN);ctx=client_tls_context();if(!ctx){set_error("cannot create client TLS context");return false;}run.store(true);generation.store(0);if(!ensure_tunnel()){run.store(false);SSL_CTX_free(ctx);ctx=nullptr;return false;}TlsConn initial;std::string token,fp;if(!connect_authenticated(true,initial,token,fp)){run.store(false);tunnel_access_stop(tunnel);SSL_CTX_free(ctx);ctx=nullptr;return false;}if(!negotiate_media(initial,token,fp,1)){close_tls(initial);run.store(false);tunnel_access_stop(tunnel);SSL_CTX_free(ctx);ctx=nullptr;return false;}install_control(initial,fp,1);control_thread=std::thread([this]{control_loop();});return true;}
