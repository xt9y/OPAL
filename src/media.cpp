...
std::string capture_command(bool gsr,int fps,int bitrate,bool audio,const std::string &portal_token_file){fps=std::clamp(fps,15,240);bitrate=std::clamp(bitrate,1000,100000);if(gsr){const char *wayland=std::getenv("WAYLAND_DISPLAY");const char *debug=std::getenv("OPAL_DEBUG");const bool portal=wayland&&*wayland;const std::string source=portal?"portal":"screen";const bool debug_enabled=debug&&*debug&&std::string(debug)!="0";std::string portal_args;if(portal&&!portal_token_file.empty()){portal_args+=" -portal-session-token-filepath "+quote_arg(portal_token_file);if(std::filesystem::exists(portal_token_file))portal_args+=" -restore-portal-session yes";}return "gpu-screen-recorder -w "+source+" -f "+std::to_string(fps)+" -fm cfr -keyint 1 -k h264 -fallback-cpu-encoding yes -bm cbr -q "+std::to_string(bitrate)+" -v no -cursor yes"+(audio?" -a default_output":"")+portal_args+" -c mkv"+(debug_enabled?"":" 2>/dev/null");}return "ffmpeg -hide_banner -loglevel error -f x11grab -draw_mouse 1 -framerate "+std::to_string(fps)+" -i ${DISPLAY:-:0.0} "+(audio?"-f pulse -i default ":"")+"-c:v libx264 -preset ultrafast -tune zerolatency -b:v "+std::to_string(bitrate)+"k -maxrate "+std::to_string(bitrate)+"k -bufsize "+std::to_string(bitrate)+"k "+(audio?"-c:a aac -b:a 128k ":"")+"-f matroska pipe:1";}
