...
SinkProcess start_sink(const std::string &command){int fds[2];if(pipe(fds)!=0)return{};cloexec(fds[0]);cloexec(fds[1]);pid_t pid=fork();if(pid<0){close(fds[0]);close(fds[1]);return{};}if(pid==0){group_child();dup2(fds[0],STDIN_FILENO);close(fds[0]);close(fds[1]);execl("/bin/sh","sh","-c",command.c_str(),static_cast<char*>(nullptr));_exit(127);}group_parent(pid);close(fds[0]);const bool compact_input=command.find("opal-input")!=std::string::npos;return{pid,fds[1],compact_input};}
bool write_sink_timeout(SinkProcess &sink,const void *data,size_t size,int timeout_ms){
    ...
    if(sink.compact_input){
        const std::string_view command(static_cast<const char*>(data),size);
        if(!encode_input_command(command,compact))return false;
        data=compact.data();size=compact.size();
    }
