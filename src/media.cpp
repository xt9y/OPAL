...
static std::string ffmpeg_h264_encoder(){static const std::string encoder=[](){const auto encoders=command_output("ffmpeg -hide_banner -encoders 2>/dev/null");for(const char*name:{"libx264","libopenh264"})if(encoders.find(std::string(" ")+name+" ")!=std::string::npos)return std::string(name);return std::string("h264");}();return encoder;}
