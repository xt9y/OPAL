                         ...
                         const std::string& portal_token_file)
{
    ...
    if (!impl_->video->start(stream, bitrate_kbps)) {
        impl_->capture_error();
        if (impl_->error.empty()) impl_->error = "native macOS video capture failed to start";
        impl_->video.reset();
        return false;
    }
    ...
    if (audio) {
        ...
        if (!impl_->audio || !impl_->audio->start()) {
            impl_->capture_error();
            if (impl_->error.empty()) impl_->error = "native macOS system audio capture failed to start";
