...
bool VideoPresenter::set_mouse_capture(bool enabled){if(!impl_||!impl_->window)return false;if(!SDL_SetWindowRelativeMouseMode(impl_->window,false))return false;if(!SDL_SetWindowMouseGrab(impl_->window,false))return false;if(!SDL_ShowCursor())return false;if(enabled)SDL_RaiseWindow(impl_->window);return true;}
