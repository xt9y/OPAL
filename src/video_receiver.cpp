...
struct VideoReceiver::Impl{
    ...
    void request_idr(){const auto now=Clock::now();if(last_idr_request.time_since_epoch().count()!=0&&now-last_idr_request<std::chrono::milliseconds(250))return;last_idr_request=now;stale.fetch_add(1);audio_output.reset_to(latest_video_ts.load());if(control_send)control_send("REQUEST_IDR "+std::to_string(path.generation));}
    void update_telemetry(const ReassembledFrame&a,double reassembly_ms,double decode_ms,double present_ms,std::uint64_t first,std::uint64_t done){const auto offset=clock_offset_us.load();const double network=std::max(0.0,static_cast<double>(static_cast<std::int64_t>(first)-static_cast<std::int64_t>(a.capture_timestamp_us)+offset)/1000.0),total=std::max(0.0,static_cast<double>(static_cast<std::int64_t>(done)-static_cast<std::int64_t>(a.capture_timestamp_us)+offset)/1000.0);telemetry.network_ms=ewma(telemetry.network_ms,network);telemetry.reassembly_ms=ewma(telemetry.reassembly_ms,reassembly_ms);telemetry.decode_ms=ewma(telemetry.decode_ms,decode_ms);telemetry.present_ms=ewma(telemetry.present_ms,present_ms);telemetry.total_ms=ewma(telemetry.total_ms,total);telemetry.stale_frames=stale.load();last_decode_age_us.store(static_cast<std::uint32_t>(std::min(4294967295.0,total*1000.0)));}
    bool decode_and_present(const ReassembledFrame&a,double reassembly_ms,std::uint64_t first){const auto decode_begin=monotonic_us();DecodedVideoView newest;std::size_t superseded=0;if(!decoder_ready||!decoder.decode_latest(a.data,static_cast<std::int64_t>(a.capture_timestamp_us),newest,superseded)){decoder.flush();request_idr();return false;}if(!newest.frame)return true;if(superseded)stale.fetch_add(superseded);const auto decode_end=monotonic_us();if(!presenter.x11_window()){bool fullscreen=true;if(const char*w=std::getenv("OPAL_VIDEO_WINDOWED");w&&*w&&std::string(w)!="0")fullscreen=false;if(!presenter.open(newest.frame->width,newest.frame->height,fullscreen)){failed.store(true);run.store(false);return false;}window.store(presenter.x11_window());}const auto present_begin=monotonic_us();if(!presenter.present_borrowed(newest)){failed.store(true);run.store(false);return false;}const auto present_end=monotonic_us();latest_video_ts.store(static_cast<std::int64_t>(a.capture_timestamp_us));media.store(true);update_telemetry(a,reassembly_ms,static_cast<double>(decode_end-decode_begin)/1000.0,static_cast<double>(present_end-present_begin)/1000.0,first,present_end);return true;}
    void handle_complete(const ReassembledFrame&a,double reassembly_ms,std::uint64_t first){if(a.media_type==VideoMediaType::VideoH264){if(a.config){decoder.flush();decoder_ready=decoder.configure_h264(a.data);if(!decoder_ready)request_idr();return;}decode_and_present(a,reassembly_ms,first);return;}if(a.media_type==VideoMediaType::AudioAac){if(a.config){if(a.data.size()<5){audio_ready=false;return;}const int rate=static_cast<int>(read32(a.data)),channels=a.data[4];audio_ready=audio_output.configure_aac(std::span<const std::uint8_t>(a.data).subspan(5),rate,channels);return;}if(audio_ready)audio_output.submit(a.data,static_cast<std::int64_t>(a.capture_timestamp_us),latest_video_ts.load());}}
