...
bool zrok_enabled(){if(!command_exists("zrok2")){std::cerr<<"OPAL requires zrok2 for control networking.\n";return false;}int p[2];if(pipe(p)!=0)return false;cloexec(p[0]);cloexec(p[1]);pid_t pid=fork();if(pid==0){dup2(p[1],STDOUT_FILENO);dup2(p[1],STDERR_FILENO);close(p[0]);close(p[1]);execlp("zrok2","zrok2","status",static_cast<char*>(nullptr));_exit(127);}close(p[1]);std::string out;char b[1024];ssize_t n;while((n=read(p[0],b,sizeof(b)))>0)out.append(b,n);close(p[0]);int st=0;waitpid(pid,&st,0);if(WIFEXITED(st)&&WEXITSTATUS(st)==0&&out.find("EnvZId")!=std::string::npos)return true;std::cout<<"zrok2 is not enabled. Paste the token from `zrok enable <token>`: "<<std::flush;std::string token;if(!std::getline(std::cin,token)||trim(token).empty())return false;token=trim(token);pid=fork();if(pid==0){execlp("zrok2","zrok2","enable",token.c_str(),static_cast<char*>(nullptr));_exit(127);}waitpid(pid,&st,0);return WIFEXITED(st)&&WEXITSTATUS(st)==0;}
}

void tunnel_access_stop(TunnelAccessHandle&h){stop(h.control_pid);h.control_port=0;}
bool tunnel_access_healthy(TunnelAccessHandle&h){return alive(h.control_pid)&&h.control_port>0&&ready(h.control_port);}
bool tunnel_access_start(TunnelAccessHandle&h,const std::string&ct,int timeout_ms){
    if(tunnel_access_healthy(h))return true;tunnel_access_stop(h);if(ct.empty()||!zrok_enabled())return false;
    auto deadline=std::chrono::steady_clock::now()+std::chrono::milliseconds(std::max(1,timeout_ms));
    while(std::chrono::steady_clock::now()<deadline){
        h.control_port=free_port();if(h.control_port<=0){tunnel_access_stop(h);continue;}
        h.control_pid=spawn({"zrok2","access","private",ct,"--bind","127.0.0.1:"+std::to_string(h.control_port),"--headless","--force-local"});
        while(std::chrono::steady_clock::now()<deadline){if(!alive(h.control_pid))break;if(ready(h.control_port))return true;std::this_thread::sleep_for(std::chrono::milliseconds(100));}
        tunnel_access_stop(h);std::this_thread::sleep_for(std::chrono::milliseconds(150));
    }
    return false;
}
}
