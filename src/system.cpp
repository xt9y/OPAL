...
int ensure_tailnet(){return require_tailscale();}
int init(){auto p=Paths::load();if(!ensure_layout(p)||!ensure_identity(p.identity_key,p.identity_pub))return 1;Ini c;if(!std::filesystem::exists(p.config)){c.set("video","fps","60");c.set("video","bitrate_kbps","30000");c.set("video","fullscreen","true");c.set("audio","enabled","true");c.set("network","mode","opal-native");c.set("network","transport","tailscale");c.save(p.config);}std::cout<<"Initialized "<<p.root<<"\n";return 0;}
int doctor(){auto p=Paths::load();std::cout<<"OPAL doctor\n";auto show=[](const char*n,bool ok){std::cout<<(ok?"[ok]   ":"[warn] ")<<n<<"\n";};std::string sdl_driver;const bool sdl_ok=sdl_video_available(sdl_driver);const bool h264_ok=h264_decoder_available();const bool native_capture=NativePipeWireVideoCapture::compiled();const bool ffmpeg_binary=command_exists("ffmpeg");const bool ffmpeg_stream=ffmpeg_binary&&!capture_command(false,60,4000,false,"",320,180).empty();const auto encoder=preferred_h264_encoder(),hw_decoder=preferred_h264_hw_decoder();show("Native PipeWire/libportal capture build",native_capture);if(!native_capture&&std::filesystem::exists("/etc/fedora-release"))std::cout<<"[info] Fedora native capture build: sudo dnf install libportal-devel pipewire-devel ffmpeg-free-devel\n";show((std::string("In-process H.264 encoder (")+(encoder.empty()?"unavailable":encoder)+")").c_str(),!encoder.empty());show((std::string("H.264 hardware decoder candidate (")+(hw_decoder.empty()?"unavailable":hw_decoder)+")").c_str(),!hw_decoder.empty());show("FFmpeg binary",ffmpeg_binary);show("FFmpeg streaming H.264 fallback",ffmpeg_stream);show("Linked FFmpeg H.264 decoder",h264_ok);if(!h264_ok&&std::filesystem::exists("/etc/fedora-release"))std::cout<<"[info] Fedora: enable RPM Fusion Free and install libavcodec-freeworld/ffmpeg-libs for H.264 decode.\n";show("GPU Screen Recorder fallback",command_exists("gpu-screen-recorder"));const std::string sdl_line="SDL3 client video backend ("+(sdl_ok?sdl_driver:std::string("unavailable"))+")";show(sdl_line.c_str(),sdl_ok);show("Wayland clipboard (wl-clipboard)",command_exists("wl-paste")&&command_exists("wl-copy"));show("PulseAudio/PipeWire audio service",command_exists("pactl")||command_exists("wpctl"));show("Tailscale installed",tailscale_cli_available());show("Tailscale connected",tailscale_connected());show("Wayland session",std::getenv("WAYLAND_DISPLAY")!=nullptr);show("/dev/uinput",access("/dev/uinput",W_OK)==0);show("~/.opal initialized",std::filesystem::exists(p.root));std::cout<<"[info] Native capture uses PipeWire capture-cycle timestamps; subprocess capture telemetry is explicitly estimated.\n";std::cout<<"[info] Hardware decode prefers DRM PRIME export and the client presenter attempts DMA-BUF/EGL import before CPU upload.\n";std::cout<<"[info] SDL3 client input replaces XInput2 raw client input.\n";std::cout<<"[info] Networking requires Tailscale; OPAL uses its own authenticated encrypted peer session over the tailnet.\n";return 0;}
