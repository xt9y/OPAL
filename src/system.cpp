...
int init(){auto p=Paths::load();if(!ensure_layout(p)||!ensure_identity(p.identity_key,p.identity_pub)||!ensure_tls_certificate(p.cert.string(),p.cert_key.string()))return 1;Ini c;if(!std::filesystem::exists(p.config)){c.set("video","fps","60");c.set("video","bitrate_kbps","30000");c.set("video","fullscreen","true");c.set("audio","enabled","true");c.set("network","mode","direct-video");c.set("network","transport","zrok2-control+direct-udp");c.save(p.config);}std::cout<<"Initialized "<<p.root<<"\n";return 0;}
int doctor(){
    auto p=Paths::load();std::cout<<"OPAL doctor\n";
    auto show=[](const char*n,bool ok){std::cout<<(ok?"[ok]   ":"[warn] ")<<n<<"\n";};
    show("OpenSSL CLI",command_exists("openssl"));
    show("FFmpeg capture fallback",command_exists("ffmpeg"));
    show("GPU Screen Recorder (preferred)",command_exists("gpu-screen-recorder"));
    show("zrok2 control transport",command_exists("zrok2"));
    show("X11/XWayland client display (GLX presenter)",std::getenv("DISPLAY")!=nullptr);
    show("XInput2 raw client input",xinput2_available());
    show("PulseAudio/PipeWire audio service",command_exists("pactl")||command_exists("wpctl"));
    show("Wayland host session",std::getenv("WAYLAND_DISPLAY")!=nullptr);
    show("/dev/uinput",access("/dev/uinput",W_OK)==0);
    show("~/.opal initialized",std::filesystem::exists(p.root));
    std::cout<<"[info] Video transport is direct encrypted UDP only; zrok is never used for media.\n";
    return 0;
}
