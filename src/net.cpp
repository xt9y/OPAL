...
void close_tls(TlsConn&c){if(c.ssl){SSL_set_quiet_shutdown(c.ssl,1);SSL_free(c.ssl);c.ssl=nullptr;}if(c.fd>=0){close(c.fd);c.fd=-1;}}
...
bool tls_read_line_timeout(SSL*s,std::string&line,int timeout_ms,size_t limit){line.clear();if(!s)return false;int fd=SSL_get_fd(s);if(fd<0)return false;int flags=fcntl(fd,F_GETFL,0);if(flags<0||fcntl(fd,F_SETFL,flags|O_NONBLOCK)<0)return false;auto restore=[&]{fcntl(fd,F_SETFL,flags);};auto deadline=std::chrono::steady_clock::now()+std::chrono::milliseconds(std::max(1,timeout_ms));char buf[1024];while(line.size()<limit){int n=SSL_peek(s,buf,static_cast<int>(std::min<size_t>(sizeof(buf),limit-line.size())));if(n>0){int take=n;for(int i=0;i<n;++i)if(buf[i]=='\n'){take=i+1;break;}char consumed[1024];int r=SSL_read(s,consumed,take);if(r<=0)continue;for(int i=0;i<r;++i){char ch=consumed[i];if(ch=='\n'){restore();return true;}if(ch!='\r')line+=ch;}continue;}int e=SSL_get_error(s,n);if(e!=SSL_ERROR_WANT_READ&&e!=SSL_ERROR_WANT_WRITE){restore();return false;}if(!wait_fd(fd,e==SSL_ERROR_WANT_WRITE?POLLOUT:POLLIN,deadline)){restore();return false;}}restore();return false;}
