...
int recv_datagrams_batch(int fd,std::span<UdpReceiveSlot> slots,int timeout_ms){
    ...
    for(std::size_t i=0;i<count;++i){
        ...
        messages[i].msg_hdr.msg_name=&slots[i].source;
        messages[i].msg_hdr.msg_namelen=sizeof(slots[i].source);
    ...
    for(int i=0;i<received;++i){
        ...
        slot.source_length=messages[static_cast<std::size_t>(i)].msg_hdr.msg_namelen;
#ifdef SO_RXQ_OVFL
        for(cmsghdr *cmsg=CMSG_FIRSTHDR(&messages[static_cast<std::size_t>(i)].msg_hdr);cmsg;cmsg=CMSG_NXTHDR(&messages[static_cast<std::size_t>(i)].msg_hdr,cmsg)){
            if(cmsg->cmsg_level==SOL_SOCKET&&cmsg->cmsg_type==SO_RXQ_OVFL&&cmsg->cmsg_len>=CMSG_LEN(sizeof(std::uint32_t))){
    ...
    for(std::size_t i=0;i<count;++i){
        auto &slot=slots[i];
        socklen_t length=sizeof(slot.source);
        const int n=recv_datagram(fd,slot.buffer,slot.source,length,i==0?timeout_ms:0);
        if(n==-2)break;
        if(n<0)return received?received:-1;
        slot.size=static_cast<std::size_t>(n);
        slot.source_length=length;
        ++received;
    }
    return received;
#endif
