...
bool resolve_udp_endpoint(const std::string &host,std::uint16_t port,sockaddr_storage &output,socklen_t &output_length){
    ...
    addrinfo hints{};hints.ai_socktype=SOCK_DGRAM;hints.ai_family=AF_UNSPEC;addrinfo *result=nullptr;
    const std::string service=std::to_string(port);if(getaddrinfo(host.c_str(),service.c_str(),&hints,&result)!=0)return false;
    bool resolved=false;
    for(auto *it=result;it&&!resolved;it=it->ai_next){
        if(it->ai_family==AF_INET6&&it->ai_addrlen<=sizeof(output)){
            std::memcpy(&output,it->ai_addr,it->ai_addrlen);output_length=static_cast<socklen_t>(it->ai_addrlen);resolved=true;
        }else if(it->ai_family==AF_INET){
            const auto *v4=reinterpret_cast<const sockaddr_in*>(it->ai_addr);sockaddr_in6 mapped{};mapped.sin6_family=AF_INET6;mapped.sin6_port=v4->sin_port;mapped.sin6_addr.s6_addr[10]=0xff;mapped.sin6_addr.s6_addr[11]=0xff;std::memcpy(mapped.sin6_addr.s6_addr+12,&v4->sin_addr,4);std::memcpy(&output,&mapped,sizeof(mapped));output_length=sizeof(mapped);resolved=true;
        }
    }
    freeaddrinfo(result);return resolved;
}

bool send_datagram(int fd,const sockaddr_storage &address,socklen_t address_length,std::span<const std::uint8_t> data){
    if(fd<0||data.empty()||data.size()>65507)return false;
    const ssize_t written=sendto(fd,data.data(),data.size(),0,reinterpret_cast<const sockaddr*>(&address),address_length);
...
int recv_datagram(int fd,std::span<std::uint8_t> data,sockaddr_storage &source,socklen_t &source_length,int timeout_ms){
    if(fd<0||data.empty())return -1;pollfd descriptor{fd,POLLIN,0};int rc=0;do{rc=poll(&descriptor,1,std::max(0,timeout_ms));}while(rc<0&&errno==EINTR);if(rc==0)return -2;if(rc<0||!(descriptor.revents&POLLIN))return -1;source_length=sizeof(source);ssize_t received=0;do{received=recvfrom(fd,data.data(),data.size(),0,reinterpret_cast<sockaddr*>(&source),&source_length);}while(received<0&&errno==EINTR);if(received<0&&(errno==EAGAIN||errno==EWOULDBLOCK))return -2;return received<0?-1:static_cast<int>(received);
}

int recv_datagrams_batch(int fd,std::span<UdpReceiveSlot> slots,int timeout_ms){
    if(fd<0||slots.empty())return -1;const std::size_t count=std::min(slots.size(),kUdpReceiveBatchMax);
    ...
    pollfd descriptor{fd,POLLIN,0};int rc=0;do{rc=poll(&descriptor,1,std::max(0,timeout_ms));}while(rc<0&&errno==EINTR);if(rc==0)return 0;if(rc<0||!(descriptor.revents&POLLIN))return -1;
    ...
    int received=0;do{received=recvmmsg(fd,messages.data(),static_cast<unsigned int>(count),MSG_DONTWAIT,nullptr);}while(received<0&&errno==EINTR);if(received<0&&(errno==EAGAIN||errno==EWOULDBLOCK))return 0;if(received<0)return -1;
