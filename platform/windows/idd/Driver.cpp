...
void fill_signal_info(DISPLAYCONFIG_VIDEO_SIGNAL_INFO& info, std::uint32_t width,
                      std::uint32_t height, std::uint32_t refresh, bool monitor)
{
    info.totalSize.cx = info.activeSize.cx = width;
    info.totalSize.cy = info.activeSize.cy = height;
    info.vSyncFreq.Numerator = refresh;
    info.vSyncFreq.Denominator = 1;
    info.hSyncFreq.Numerator = refresh * height;
    info.hSyncFreq.Denominator = 1;
    info.pixelRate = static_cast<UINT64>(refresh) * width * height;
...
IDDCX_MONITOR_MODE monitor_mode(const DisplayModeRequest& mode)
{
    IDDCX_MONITOR_MODE result{};
    result.Size = sizeof(result);
    result.Origin = IDDCX_MONITOR_MODE_ORIGIN_DRIVER;
    fill_signal_info(result.MonitorVideoSignalInfo, mode.width, mode.height, mode.refresh_hz, true);
    return result;
}

IDDCX_TARGET_MODE target_mode(const DisplayModeRequest& mode)
{
    IDDCX_TARGET_MODE result{};
    result.Size = sizeof(result);
    fill_signal_info(result.TargetVideoSignalInfo.targetVideoSignalInfo,
                     mode.width, mode.height, mode.refresh_hz, false);
    return result;
}
...
bool FrameStore::publish(ID3D11Texture2D* surface, ID3D11Device* device, ID3D11DeviceContext* context)
{
    ...
    if (desc.Width == 0 || desc.Height == 0 || desc.Width > kMaxWidth || desc.Height > kMaxHeight ||
    ...
    if (!staging_ || staging_width_ != desc.Width || staging_height_ != desc.Height || staging_format_ != desc.Format) {
        ...
        if (FAILED(device->CreateTexture2D(&staging_desc, nullptr, &staging_)) || !staging_) return false;
    ...
    std::vector<std::uint8_t> next(static_cast<std::size_t>(stride) * desc.Height);
    const auto* source = static_cast<const std::uint8_t*>(mapped.pData);
    for (std::uint32_t y = 0; y < desc.Height; ++y)
        std::memcpy(next.data() + static_cast<std::size_t>(y) * stride,
...
void SwapChainProcessor::run()
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = IddCxSwapChainSetDevice(swapchain_, &input);
    }
    ...
    while (SUCCEEDED(hr)) {
        ...
        if (hr == E_PENDING) {
            ...
            const DWORD result = WaitForMultipleObjects(2, waits, FALSE, 16);
            if (result == WAIT_OBJECT_0 || result == WAIT_TIMEOUT) continue;
...
NTSTATUS DeviceContext::create_monitor(const DisplayModeRequest& mode)
{
    ...
    if (monitor_) return set_mode(mode);
...
EVT_WDF_OBJECT_CONTEXT_CLEANUP OpalDeviceCleanup;
EVT_WDF_OBJECT_CONTEXT_CLEANUP OpalMonitorCleanup;
...
NTSTATUS OpalDeviceAdd(WDFDRIVER driver, PWDFDEVICE_INIT init)
{
    ...
    WDF_OBJECT_ATTRIBUTES attributes;
    WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE(&attributes, DeviceContextWrapper);
    attributes.EvtCleanupCallback = OpalDeviceCleanup;
...
_Use_decl_annotations_
VOID OpalDeviceCleanup(WDFOBJECT object)
{
...
_Use_decl_annotations_
VOID OpalMonitorCleanup(WDFOBJECT object)
{
                         ...
                         size_t input_length, ULONG code)
{
    ...
    if (context && code == kIoctlGetStatus) {
        ...
        if (NT_SUCCESS(status)) {
            const auto request_data = *static_cast<const FrameRequest*>(input);
            if (request_data.version != kProtocolVersion) status = STATUS_REVISION_MISMATCH;
            else status = context->frames()->copy_if_new(request_data.last_sequence, output,
                                    ...
                                    IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* output)
{
    ...
    input->pDefaultMonitorModes[0] = monitor_mode(wrapper->value->mode());
                                     ...
                                     IDARG_OUT_QUERYTARGETMODES* output)
{
    ...
    input->pTargetModes[0] = target_mode(wrapper->value->mode());
    return STATUS_SUCCESS;
}
