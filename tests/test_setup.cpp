...
int init_calls=0,ensure_tailnet_calls=0,host_setup_calls=0,host_run_calls=0,host_service_calls=0,client_calls=0,wake_calls=0,add_calls=0;std::string client_target,added_address,wake_target;StreamOptions client_stream;
int init(){++init_calls;ensure_layout(Paths::load());return 0;}int ensure_tailnet(){++ensure_tailnet_calls;return 0;}int doctor(){return 0;}int host_setup(){++host_setup_calls;ensure_layout(Paths::load());Ini h;h.set("host","password","TEST");h.save(Paths::load().host);std::cout<<"OPAL host ready.\n";return 0;}int host_run(){++host_run_calls;return 0;}int host_service(bool){++host_service_calls;return 0;}int bridge_setup(const char*){return 0;}int hosts_list(){return 0;}
...
static void reset_calls(){opal::init_calls=opal::ensure_tailnet_calls=opal::host_setup_calls=opal::host_run_calls=opal::host_service_calls=opal::client_calls=opal::wake_calls=opal::add_calls=0;opal::client_target.clear();opal::added_address.clear();opal::wake_target.clear();opal::client_stream={};}
...
int main(){const auto native_code=code();assert(native_code.rfind("opal:",0)!=0);assert(native_code.size()==14);
    {auto p=fresh("opal-setup-host-test");reset_calls();assert(run_default("1\ny\n")==0);opal::Ini c;assert(c.load(p/"config.ini"));assert(c.get("opal","role")=="host");assert(opal::ensure_tailnet_calls==1&&opal::host_setup_calls==1&&opal::host_service_calls==1&&opal::host_run_calls==0);}
    {auto p=fresh("opal-setup-client-test");reset_calls();assert(run_default("2\n"+native_code+"\ndesktop\n")==0);opal::Ini c;assert(c.load(p/"config.ini"));assert(c.get("opal","role")=="client"&&c.get("opal","default_host")=="desktop");assert(opal::ensure_tailnet_calls==1&&opal::add_calls==1&&opal::added_address==native_code&&opal::client_calls==1&&opal::client_target=="desktop");assert(opal::client_stream.max_width==1920&&opal::client_stream.max_height==1080&&opal::client_stream.fps==60);}
    {auto p=fresh("opal-setup-prefixed-code-test");reset_calls();assert(run_default("2\nopal:"+native_code+"\nlegacy\n")==0);opal::Ini c;assert(c.load(p/"config.ini"));assert(c.get("opal","role")=="client"&&c.get("opal","default_host")=="legacy");assert(opal::add_calls==1&&opal::added_address=="opal:"+native_code&&opal::client_calls==1&&opal::client_target=="legacy");}
    {fresh("opal-setup-legacy-code-test");reset_calls();std::string err;assert(run_default("2\nopal:control-token,legacy-video-token\n",nullptr,&err)==2);assert(err.find("XXXX-XXXX-XXXX")!=std::string::npos);assert(err.find("opal:XXXX-XXXX-XXXX")==std::string::npos);assert(opal::ensure_tailnet_calls==1&&opal::add_calls==0&&opal::client_calls==0);}
    {fresh("opal-setup-invalid-code-test");reset_calls();std::string err;assert(run_default("2\nopal,opal-ctl-bad,opal-vid-bad\n",nullptr,&err)==2);assert(err.find("XXXX-XXXX-XXXX")!=std::string::npos);assert(err.find("opal:XXXX-XXXX-XXXX")==std::string::npos);assert(opal::ensure_tailnet_calls==1&&opal::add_calls==0&&opal::client_calls==0);}
    {auto p=fresh("opal-auto-host-test");reset_calls();opal::Ini c;c.set("opal","role","host");c.save(p/"config.ini");assert(run_default("")==0);assert(opal::ensure_tailnet_calls==1&&opal::host_service_calls==1&&opal::host_run_calls==0);}
    {auto p=fresh("opal-auto-client-test");reset_calls();opal::Ini c;c.set("opal","role","client");c.set("opal","default_host","desktop");c.save(p/"config.ini");opal::Ini h;std::string id;assert(opal::parse_connection_code(native_code,id));h.set("desktop","rendezvous_id",id);h.set("desktop","mac","00:11:22:33:44:55");h.save(p/"hosts.ini");assert(run_default("")==0);assert(opal::ensure_tailnet_calls==1&&opal::wake_calls==1&&opal::wake_target=="desktop"&&opal::client_calls==1&&opal::client_target=="desktop");}
    {auto p=fresh("opal-stream-override-test");reset_calls();opal::Ini c;c.set("opal","role","client");c.set("opal","default_host","desktop");c.save(p/"config.ini");opal::Ini h;std::string id;opal::parse_connection_code(native_code,id);h.set("desktop","rendezvous_id",id);h.save(p/"hosts.ini");opal::StreamOptions stream{2560,1440,120};assert(run_default_stream(stream,"")==0);assert(opal::ensure_tailnet_calls==1&&opal::client_calls==1&&opal::client_stream.max_width==2560&&opal::client_stream.max_height==1440&&opal::client_stream.fps==120);}
