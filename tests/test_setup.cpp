...
namespace opal {
int init_calls=0,host_setup_calls=0,host_run_calls=0,host_service_calls=0,client_calls=0,wake_calls=0,add_calls=0;
...
int hosts_add(const std::string&name,const std::string&address,const std::string&mac){++add_calls;Ini h;h.load(Paths::load().hosts);h.set(name,"address",address);h.set(name,"port","9");if(!mac.empty())h.set(name,"mac",mac);h.save(Paths::load().hosts);return 0;}
...
static void reset_calls(){
    opal::init_calls=opal::host_setup_calls=opal::host_run_calls=opal::host_service_calls=opal::client_calls=opal::wake_calls=opal::add_calls=0;
    opal::client_target.clear();
}
...
int main(){
    {
        ...
        assert(opal::host_setup_calls==1);assert(opal::host_run_calls==1);
        assert(out.find("opal host")==std::string::npos);
    }
    {
        ...
        assert(opal::add_calls==1);assert(opal::client_calls==1);assert(opal::client_target=="desktop");
    }
    {
        auto p=fresh("opal-auto-host-test");reset_calls();opal::Ini c;c.set("opal","role","host");c.save(p/"config.ini");
        assert(run_with("")==0);assert(opal::host_run_calls==1);
    }
    {
        auto p=fresh("opal-wake-test");reset_calls();
        opal::Ini c;c.set("opal","role","client");c.set("opal","default_host","desktop");c.set("network","connect_wait_seconds","0");c.save(p/"config.ini");
        opal::Ini h;h.set("desktop","address","127.0.0.1");h.set("desktop","port","9");h.set("desktop","mac","00:11:22:33:44:55");h.save(p/"hosts.ini");
        assert(run_with("")==0);assert(opal::wake_calls==1);assert(opal::client_calls==1);
    }
