...
std::vector<std::uint8_t> payload_for(std::uint32_t generation,int frame,bool keyframe){
    const std::size_t size=keyframe?6500u:3600u+static_cast<std::size_t>((generation*31+frame*17)%1800);
...
int main(){
    ...
    auto keys=test_keys();std::mt19937 rng(0x0bad5eedu);std::uniform_int_distribution<int> loss_percent(0,5),coin(0,99);
    std::uint64_t stale_drops=0;bool saw_single_fec=false,saw_two_loss=false,saw_need_idr=false,saw_recovery=false;
    ...
    for(std::uint32_t generation=1;generation<=50;++generation){
        ...
        opal::ReplayWindow1024 replay;std::uint64_t sequence=base_sequence+static_cast<std::uint64_t>(generation)*100000;
    ...
    assert(saw_single_fec&&saw_two_loss&&saw_need_idr&&saw_recovery);
    return 0;
}
