...
int main(){
    ...
    auto packets=opal::fragment_media_unit(opal::VideoMediaType::VideoH264,opal::FrameKeyframe,
        5,77,1,16667,expected,sequence,true);
    opal::VideoReassembler reassembler;reassembler.reset(5,77);
    ...
    for(const auto &packet:packets){
        ...
        auto status=reassembler.accept(packet,complete);
        if(status==opal::ReassemblyStatus::Complete){completed=true;break;}
    }
    assert(completed&&complete.data==expected&&complete.frame_id==1&&complete.keyframe);

    reassembler.reset(5,77);completed=false;
    for(const auto &packet:packets){
        if(packet.header.media_type!=opal::VideoMediaType::Fec&&
           (packet.header.fragment_index==2||packet.header.fragment_index==3))continue;
    ...
    for(std::uint64_t id=10;id<14;++id){
        ...
        for(const auto &packet:frame){
            if(packet.header.media_type!=opal::VideoMediaType::Fec&&packet.header.fragment_index==0){
                fourth=reassembler.accept(packet,complete);break;
            }
        }
    }
    ...
    oversized.header.generation=5;oversized.header.session_id=77;oversized.header.frame_id=20;
    oversized.header.fragment_count=1;oversized.header.payload_length=static_cast<std::uint16_t>(opal::kVideoPlaintextBytes);
    ...
    reassembler.reset(6,88);assert(reassembler.frames_in_flight()==0&&reassembler.bytes_in_flight()==0);
