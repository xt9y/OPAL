...
int main(){
    ...
    assert(source.find("AVPacket *packet=av_packet_alloc()") == std::string::npos);
    assert(source.find("AVFrame *frame=av_frame_alloc()") == std::string::npos);
    ...
    opal::EncodedMediaUnit unit,last_audio;int submitted=0;
    for(int i=0;i<100&&submitted<4;++i){if(!capture.next(unit,1000))continue;if(unit.kind!=opal::MediaKind::AudioAac)continue;last_audio=unit;assert(output.submit(unit.data,unit.pts_us,0));++submitted;assert(output.queued_ms()<=40);}
