...
int main(){
    ...
    opal::PeerSessionOptions host_options;host_options.client_side=false;host_options.socket=host_socket;host_options.peer={"198.51.100.20",client_socket.local_port};host_options.lan_peer=opal::RendezvousEndpoint{"::1",client_socket.local_port};host_options.handshake=context;host_options.identity_private_key=host_id.priv;host_options.reliable_input=[&](const std::string&s){std::lock_guard<std::mutex>l(mu);host_reliable.push_back(s);};host_options.pointer_input=[&](const std::string&s){std::lock_guard<std::mutex>l(mu);host_pointer.push_back(s);};host_options.media_datagram=[&](std::span<const std::uint8_t>b){std::lock_guard<std::mutex>l(mu);host_media.assign(b.begin(),b.end());};
