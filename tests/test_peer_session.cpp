...
int main(){
    ...
    std::mutex mu;std::vector<std::string>host_reliable,host_pointer;std::vector<std::uint8_t>host_media;std::atomic<bool>slow_media_started{false};
    opal::PeerSessionOptions host_options;host_options.client_side=false;host_options.socket=host_socket;host_options.peer={"198.51.100.20",client_socket.local_port};host_options.lan_peer=opal::RendezvousEndpoint{"::1",client_socket.local_port};host_options.handshake=context;host_options.identity_private_key=host_id.priv;host_options.reliable_input=[&](const std::string&s){std::lock_guard<std::mutex>l(mu);host_reliable.push_back(s);};host_options.pointer_input=[&](const std::string&s){std::lock_guard<std::mutex>l(mu);host_pointer.push_back(s);};host_options.media_datagram=[&](std::span<const std::uint8_t>b){if(b.size()==1&&b[0]==0x7f){slow_media_started.store(true);std::this_thread::sleep_for(std::chrono::milliseconds(3300));return;}std::lock_guard<std::mutex>l(mu);host_media.assign(b.begin(),b.end());};
