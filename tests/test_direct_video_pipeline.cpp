...
void wire_control(opal::VideoSender &sender,opal::VideoReceiver &receiver,std::atomic<int> &idr_requests,opal::DirectVideoPath sender_path,opal::DirectVideoPath receiver_path){assert(sender.start(std::move(sender_path),{320,180,60},false,[&](const std::string &line){assert(receiver.handle_control_line(line));}));assert(receiver.start(std::move(receiver_path),[&](const std::string &line){if(line.rfind("REQUEST_IDR ",0)==0)++idr_requests;assert(sender.handle_control_line(line));}));}
void run_case(int dropped_fragments,bool expect_idr,std::uint32_t generation){setenv("OPAL_TEST_DROP_FRAGMENTS",std::to_string(dropped_fragments).c_str(),1);auto sender_socket=opal::open_udp_socket(),receiver_socket=opal::open_udp_socket();assert(sender_socket.fd>=0&&receiver_socket.fd>=0);auto sender_path=make_path(sender_socket,receiver_socket.local_port,true,generation);auto receiver_path=make_path(receiver_socket,sender_socket.local_port,false,generation);opal::VideoSender sender;opal::VideoReceiver receiver;std::atomic<int> idr_requests{0};wire_control(sender,receiver,idr_requests,std::move(sender_path),std::move(receiver_path));const auto deadline=std::chrono::steady_clock::now()+std::chrono::seconds(8);while(!receiver.media_started()&&std::chrono::steady_clock::now()<deadline)std::this_thread::sleep_for(std::chrono::milliseconds(10));assert(receiver.media_started());assert(receiver.presentation_window()!=0);assert(sender.queued_frames()<=2&&sender.queued_bytes()<=16u*1024u*1024u);assert(opal::kVideoMaxDatagramBytes==1200);if(expect_idr)assert(idr_requests.load()>=1);else assert(idr_requests.load()==0);sender.stop();receiver.stop();}
