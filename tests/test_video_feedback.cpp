...
int main(){
    ...
    host.stale_frames=2;host.idr_requests=3;host.restarts=1;host.chain_valid=true;host.restart_reason="decode-backlog";host.capture_timestamp_exact=true;
    ...
    assert(host_parsed.restart_reason=="decode-backlog"&&host_parsed.capture_timestamp_exact);
    ...
    auto invalid_bool=host_line;const auto bool_pos=invalid_bool.find(" 2 3 1 1 ");assert(bool_pos!=std::string::npos);invalid_bool.replace(bool_pos+7,1,"2");
    ...
    assert(human.find("capture_clock=exact")!=std::string::npos);
    ...
    latency.decoded_fps=59.4;latency.presented_fps=59.1;latency.video_queue_depth=2;latency.skipped_present_frames=7;latency.capture_timestamp_exact=true;
    const auto latency_line=opal::format_latency_telemetry(latency);
    assert(latency_line.find("capture_clock=exact")!=std::string::npos);
