...
static void no_legacy_networking(const std::string&text){assert(text.find("zrok2")==std::string::npos);assert(text.find("tunnel_access")==std::string::npos);assert(text.find("tunnel_host_start")==std::string::npos);assert(text.find("TunnelAccessHandle")==std::string::npos);}
int main(){
    ...
    assert(session.find("RendezvousClient")!=std::string::npos&&session.find("PeerSession")!=std::string::npos&&session.find("start_native")!=std::string::npos&&session.find("connect_tls")==std::string::npos);
    const auto peer=read_all("src/peer_session.cpp");assert(peer.find("direct_handshake_timeout_ms")!=std::string::npos&&peer.find("relay_handshake_timeout_ms")!=std::string::npos&&peer.find("wrap_relay_datagram")!=std::string::npos&&peer.find("path=\"direct\"")!=std::string::npos&&peer.find("path=\"relay\"")!=std::string::npos);
    ...
    const auto readme=read_all("README.md");no_legacy_networking(readme);assert(readme.find("opal:XXXX-XXXX-XXXX")!=std::string::npos);assert(readme.find("direct")!=std::string::npos&&readme.find("relay")!=std::string::npos);
    assert(makefile.find("rendezvous-server")!=std::string::npos&&makefile.find("test-peer-session")!=std::string::npos&&makefile.find("test-rendezvous-server")!=std::string::npos);
    const auto ci=read_all(".github/workflows/ci.yml");assert(ci.find("workflow_dispatch")!=std::string::npos);assert(ci.find("\n  push:")==std::string::npos);assert(ci.find("sanitize")!=std::string::npos&&ci.find("stress")!=std::string::npos);
    return 0;
}
