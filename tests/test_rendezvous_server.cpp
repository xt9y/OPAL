...
int main(){
    ...
    opal::RendezvousEndpoint host_lan{"192.168.1.10",41000},client_lan{"192.168.1.20",42000};
    ...
    opal::RendezvousMessage intro;intro.type=opal::RendezvousType::Introduce;intro.id=host.id;intro.public_key=client.public_hex;intro.nonce=opal::random_hex(16);intro.local_host=client_lan.host;intro.local_port=client_lan.port;out=state.process(intro,client_ep,1400);assert(out.size()==1&&out[0].target==host_ep&&out[0].message.type==opal::RendezvousType::Offer);const auto offer=out[0].message;assert(offer.host==client_ep.host&&offer.port==client_ep.port&&offer.public_key==client.public_hex&&offer.nonce==intro.nonce);assert(offer.local_host==client_lan.host&&offer.local_port==client_lan.port);assert(state.pending_introductions(1400)==1);

    opal::RendezvousMessage attacker_accept;attacker_accept.type=opal::RendezvousType::Accept;attacker_accept.id=host.id;attacker_accept.session_id=offer.session_id;attacker_accept.public_key=host.public_hex;attacker_accept.nonce=opal::random_hex(16);attacker_accept.local_host=host_lan.host;attacker_accept.local_port=host_lan.port;attacker_accept.signature=opal::sign_hex(host.priv,opal::rendezvous_accept_transcript(host.id,offer.session_id,client.public_hex,intro.nonce,attacker_accept.nonce));out=state.process(attacker_accept,attacker_ep,1450);assert(out.size()==1&&out[0].target==attacker_ep&&out[0].message.type==opal::RendezvousType::Error);
    opal::RendezvousMessage accept=attacker_accept;accept.nonce=opal::random_hex(16);accept.signature=opal::sign_hex(host.priv,opal::rendezvous_accept_transcript(host.id,offer.session_id,client.public_hex,intro.nonce,accept.nonce));out=state.process(accept,host_ep,1500);assert(out.size()==1&&out[0].target==client_ep&&out[0].message.type==opal::RendezvousType::Ready);const auto ready=out[0].message;assert(ready.id==host.id&&ready.session_id==offer.session_id&&ready.public_key==host.public_hex&&ready.host==host_ep.host&&ready.port==host_ep.port&&ready.nonce==accept.nonce);assert(ready.local_host==host_lan.host&&ready.local_port==host_lan.port);
