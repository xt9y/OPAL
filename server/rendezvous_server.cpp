...
int bind_udp(const std::string&host,std::uint16_t port){
    addrinfo hints{};hints.ai_family=AF_UNSPEC;hints.ai_socktype=SOCK_DGRAM;hints.ai_flags=AI_PASSIVE;addrinfo*result=nullptr;const auto service=std::to_string(port);if(getaddrinfo(host.empty()?nullptr:host.c_str(),service.c_str(),&hints,&result)!=0)return -1;int fd=-1;
    for(auto*it=result;it&&fd<0;it=it->ai_next){int candidate=socket(it->ai_family,SOCK_DGRAM|SOCK_CLOEXEC,0);if(candidate<0)continue;int one=1;setsockopt(candidate,SOL_SOCKET,SO_REUSEADDR,&one,sizeof(one));if(it->ai_family==AF_INET6){int off=0;setsockopt(candidate,IPPROTO_IPV6,IPV6_V6ONLY,&off,sizeof(off));}if(bind(candidate,it->ai_addr,it->ai_addrlen)==0)fd=candidate;else close(candidate);}freeaddrinfo(result);return fd;
}

bool endpoint_from_sockaddr(const sockaddr_storage&source,socklen_t length,opal::RendezvousEndpoint&endpoint){
...
bool sockaddr_from_endpoint(const opal::RendezvousEndpoint&endpoint,sockaddr_storage&out,socklen_t&length){
...
int main(int argc,char**argv){
    ...
    const int fd=bind_udp(bind_host,port);if(fd<0){std::cerr<<"cannot bind OPAL rendezvous server\n";return 1;}signal(SIGINT,stop_signal);signal(SIGTERM,stop_signal);opal::RendezvousServerState state;std::cout<<"OPAL rendezvous listening on "<<bind_host<<":"<<port<<"\n"<<std::flush;
    while(running.load()){
        std::uint8_t buffer[opal::kRendezvousMaxMessageBytes+1]{};sockaddr_storage source{};socklen_t source_len=sizeof(source);const ssize_t n=recvfrom(fd,buffer,sizeof(buffer),0,reinterpret_cast<sockaddr*>(&source),&source_len);if(n<0){if(errno==EINTR)continue;break;}if(n<=0||n>static_cast<ssize_t>(opal::kRendezvousMaxMessageBytes))continue;opal::RendezvousMessage message;if(!opal::parse_rendezvous_message(std::string_view(reinterpret_cast<const char*>(buffer),static_cast<std::size_t>(n)),message))continue;opal::RendezvousEndpoint endpoint;if(!endpoint_from_sockaddr(source,source_len,endpoint))continue;const auto outputs=state.process(message,endpoint,monotonic_ms());for(const auto&output:outputs){const auto wire=opal::serialize_rendezvous_message(output.message);if(wire.empty())continue;sockaddr_storage target{};socklen_t target_len=0;if(!sockaddr_from_endpoint(output.target,target,target_len))continue;(void)sendto(fd,wire.data(),wire.size(),0,reinterpret_cast<const sockaddr*>(&target),target_len);}
    }
    close(fd);return 0;
}
